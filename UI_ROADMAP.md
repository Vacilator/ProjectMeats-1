# ARCHIVED - Reference Only - See MASTER_PLAN.md

## UI Roadmap (Historical Snapshot)

**Authority:** This is **REFERENCE ONLY**. For current truth, backlog, and shipped evidence, see **`MASTER_PLAN.md` (canonical)**.

If anything here conflicts with the Master Plan, treat this file as outdated/historical context.

---

## Near-term UI execution (reference)

- Phase 35 frontend surface simplification is now tracked canonically in `MASTER_PLAN.md` + `.github/EPIC_TICKETS.md`; that lane supersedes older Cockpit/Command Center planning in this reference file.
- The next operator-UI simplification lane is now tracked canonically as **Phase 37** in `MASTER_PLAN.md` + `.github/EPIC_TICKETS.md`; use that lane for operator-surface completion, inline-style extraction, and workspace-cleanup work instead of reviving old ProcessCockpit planning here.
- Sprint 1 stability gate (see `MASTER_PLAN.md`): mobile viewport hardening + canonical error presentation.
- AI document surfaces (reference): backend email/document hardening is now shipped through PR #4739, and the next UI-facing follow-up is compact provenance + parse-status rendering in the AI widget/document surfaces (e.g. `Manual upload` vs `Outlook attachment`, plus retryable parser failure badges).

## Vanguard UI Templates (Shipped)

- Vanguard: Cockpit-Detail-View-Template – Complete
