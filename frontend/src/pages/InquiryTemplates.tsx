...
const InquiryTemplates: React.FC = () => {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get('/api/v1/inquiry-templates/', { params });
  ...
  const handleDeleteClick = async (template: InquiryTemplate) => {
    ...
    try {
      await axios.delete(`/api/v1/inquiry-templates/${template.id}/`);
  ...
  const handleToggleActive = async (template: InquiryTemplate) => {
    try {
      const response = await axios.patch(`/api/v1/inquiry-templates/${template.id}/`, {
