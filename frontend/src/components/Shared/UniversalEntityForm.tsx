...
export const augmentSchemaForFrontend = (
  ...
  if (normalizedEntityKey === 'location') {
    ...
    const locationPresentation: Record<string, FieldPresentationOverride> = {
      name: { section: 'General', label: 'Location Name' },
      location_type: { section: 'General', label: 'Location Type' },
      address: { section: 'Address' },
      city: { section: 'Address' },
      state: { section: 'Address' },
      zip_code: { section: 'Address' },
      country: { section: 'Address' },
      notes: { section: 'Notes', widget: 'textarea' },
    };
    ...
    return {
      ...schema,
      name: schema.name || 'Location',
      key_fields: schema.key_fields && schema.key_fields.length ? schema.key_fields : preferredKeys,
      fields: reorderFieldsByPreferredKeys(
        nextFields.map((field) =>
          applyFieldPresentation(
            field,
            locationPresentation[String(field.key || '').toLowerCase()]
          )
        ),
        preferredKeys
      ),
  ...
  useEffect(() => {
    ...
    const loadMasterProductOptions = async () => {
      try {
        ...
        logger.error('[UniversalEntityForm] Failed to load master product dropdown options:', error);
      }
    };
  ...
  const submit = useCallback(
    async (data: Record<string, unknown>) => {
      ...
      try {
        ...
        logger.error('[UniversalEntityForm] Submit failed:', err);
        const typed = err as {
          response?: {
            data?: unknown;
          };
  ...
  const fetchProducts = useCallback(
    async (fieldKey: string, q: string) => {
      ...
      try {
        ...
        logger.error('[UniversalEntityForm] Failed to search products:', err);
      } finally {
