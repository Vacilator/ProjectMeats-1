  ...
  const fetchLocations = async () => {
    try {
      ...
      if (status === 403) {
        ...
        logger.error('RLS policy rejected request', { component: 'LocationSelector' }, err);
      } else if (status === 401) {
        setFetchError('Authentication required');
        logger.error('Not authenticated', { component: 'LocationSelector' }, err);
      } else if (code === 'ECONNABORTED') {
        setFetchError('Request timeout - please try again');
      } else {
        setFetchError(getErrorMessage(err, 'Failed to load locations'));
        logger.error('Error fetching locations', { component: 'LocationSelector' }, err);
      }
    } finally {
      setLoading(false);
    }
