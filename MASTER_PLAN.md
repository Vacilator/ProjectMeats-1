...
- **UX-37.4 Shipped** — Workspace terminology alignment landed in PR #pending: standardized the surviving secondary workspace wording around **Workspace Dashboard** across Header onboarding, Cockpit page titles, tour copy, onboarding empty states, widget CTAs, and cockpit record breadcrumbs without changing routes or tour logic.
- **UX-37.5 Ready** — AICommandCenter shortcut/deep-link/confidence regression hardening: add focused coverage for Alt+1-4 shortcuts, `?item=` modal routing, confidence thresholds, and overview inbox-preview limits.
