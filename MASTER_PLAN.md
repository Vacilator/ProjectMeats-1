...
**Primary Focus**: Phase 12 execution remains active with `EH-04.1 tenant-aware-query-keys-and-cache-clear-removal` now promoted as the single top-most ready ticket after `EH-03.2 openapi-ts-mobile-typegen` was revalidated as already shipped on `development` via PRs #4777 and #4562; Phase 15 B2B Network planning and Phase 16 Core Trading Engine planning remain sealed  
...
- **Primary execution focus (P0):** continue the Phase 12 enterprise hardening lane from the top of `.github/EPIC_TICKETS.md`, starting with `EH-04.1 tenant-aware-query-keys-and-cache-clear-removal`, now that `EH-03.2 openapi-ts-mobile-typegen` has been revalidated as already shipped on `development` via PRs #4777 and #4562.
...
- **Phase 12 - Enterprise Hardening & Tech Debt Eradication (active execution lane)**
  - **Next ready ticket:** `EH-04.1 tenant-aware-query-keys-and-cache-clear-removal`
  - **Why now:** the first shared OpenAPI type generation + adoption slice is already in place, so the next remaining frontend trust-boundary gap is tenant-aware query keying without the global cache-clear workaround.
  - **Near-term execution ordering:** `EH-04.1`, then continue through the remaining `EH-04.*` through `EH-06.*` foundations in backlog order.
  - **Definition of ready:** `.github/EPIC_TICKETS.md` exposes exactly one first unchecked `Ready` ticket (`EH-04.1`) and keeps later hardening / planning tickets blocked or queued behind it.
