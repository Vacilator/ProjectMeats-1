...
- **UX-37.3 Shipped** — Cockpit workspace search/catalog cleanup landed in PR pending: replaced the Cockpit SmartSearch forwarding stub with static Command Center search guidance and removed `AILearningMetricsWidget` from new widget-catalog additions while preserving render support for existing saved layouts.
- **UX-37.4 Ready** — Workspace terminology alignment: finish the surviving Header/Cockpit tour/workspace wording cleanup so the secondary workspace terminology is consistent everywhere it remains user-facing.
