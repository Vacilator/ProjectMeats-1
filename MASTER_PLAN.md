...
- **Newly shipped since last snapshot (evidence; see `.github/MASTER_PLAN.md`)**:
  ...
  - Backend tenant safety: fail-closed `current/current_theme/admin_permissions` when tenant context is missing/ambiguous (PR #4656); wrap tenant-scoped Celery ORM in `tenant_rls(..., strict=False)` (PR #4657); and remove the last supplier create-time `TenantUser` fallback so ambiguous multi-tenant writes fail closed while single-membership middleware defaults continue to work (PR #4755).
