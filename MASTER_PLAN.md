...
- **Supplier → Plant schema cascade hardening: SHIPPED.** Breadcrumb-derived entity hierarchy is now published through `NavigationContext`, shared create flows auto-cascade and lock parent FK values instead of page-local plant defaults, supplier record/list “New Plant” entrypoints reuse the same modal path, and `Plant.site_code` is now exposed through the additive model/serializer/migration contract. 12 files + 1 migration (PR #5334).
