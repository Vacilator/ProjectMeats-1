...
- **UX-36.2 Ready** — WorkForms Monitoring single drill-in surface: simplify `/workforms/monitoring` so it is execution analytics + active-run drill-in only, not a competing queue shell.
...
- **Phase 36 is now the active execution lane.** UX-36.1 is shipped: Command Center is down to four top-level sections, legacy `tab=workflows` entrypoints now hand off to WorkForms Monitoring, and the next simplification target is UX-36.2 WorkForms Monitoring drill-in cleanup before ProcessCockpit-era retirement work.
