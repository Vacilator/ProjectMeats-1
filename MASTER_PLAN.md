...
- **UX-37.2 Ready** — AICommandCenter confidence-pill + intent-label + record-button extraction: finish the remaining inline-style removal in `AICommandCenter.tsx` and bring the confidence/shortcut surfaces under explicit tokenized wrappers.
- **UX-37.3 Blocked** — Cockpit workspace search/catalog cleanup: remove the `SmartSearch` forwarding stub from `CockpitDashboard.tsx` and keep `AILearningMetricsWidget` render-safe for existing layouts while removing it from new catalog additions.
...
- **Phase 37 is now the active execution lane.** UX-37.1 is shipped, so the remaining frontend simplification work is concentrated in the second half of `AICommandCenter` plus the surviving Cockpit workspace cleanup: extracting the confidence/intent/record inline styles, removing the Cockpit SmartSearch/catalog noise that no longer serves a distinct purpose, and hardening the shortcut/deep-link/confidence regressions that now define the Command Center-first flow.
...
- **Primary execution focus (P0):** Platform is investor-demo-ready and production-qualified, and the next highest-leverage lane is now **Phase 37 operator surface completion**. The next concrete task is **UX-37.2 AICommandCenter confidence/intention/record-button inline-style extraction** on top of the newly shipped UX-37.1 baseline.
