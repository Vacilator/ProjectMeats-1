...
- **AUTO-38.2 Ready** — AI Inbox provenance/parse-status/retryability badges: surface the new backend contract in the AI widget/document/operator review surfaces so operators can understand source + retryability at a glance.
...
- **Primary execution focus (P0):** Platform is investor-demo-ready and production-qualified, and the next highest-leverage lane is now **Phase 38 email automation reliability and operator diagnostics**. **AUTO-38.1** is now shipped on `development`, so the next concrete task is **AUTO-38.2 AI Inbox provenance/parse-status/retryability badges** using the newly landed backend contract.
