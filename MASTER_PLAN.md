...
### Phase 36: Operator Execution Surface Simplification ✅ COMPLETE
...
- **UX-36.1 Shipped** — Command Center four-section compression + Workflows demotion landed in PR #5315: removed the top-level Workflows tab from `/command-center`, added explicit execution-monitoring drill-in entrypoints, and redirected surviving `tab=workflows` entrypoints to `/workforms/monitoring` while preserving the rest of the query string.
- **UX-36.2 Shipped** — WorkForms Monitoring single drill-in surface landed in PR #5316: removed the embedded legacy submission queue shell from `/workforms/monitoring`, kept the page focused on execution analytics and active runs, and added explicit follow-through actions back to Command Center, WorkForms History, and WorkForms Catalog.
- **UX-36.3 Shipped** — ProcessCockpit legacy retirement + secondary copy alignment landed in PR #5317: retired the dead `ProcessCockpitPage` and `ProcessMonitor` surfaces plus their dead-only tests, kept legacy `/process-cockpit` redirects landing safely in Command Center, and aligned the surviving Cockpit workspace labels, onboarding copy, and breadcrumbs with the Command Center-first model.
...
- **Deliverables + expected results:**
  ...
  2. **Cockpit workspace cleanup** — remove the SmartSearch forwarding stub and stop offering new `AILearningMetrics` catalog additions while preserving saved-layout rendering; result: less cognitive noise in the secondary workspace.
  3. **Workspace terminology + test hardening** — align the surviving workspace labels/tours with the Command Center-first model and add focused regression coverage for shortcuts, `?item=` drill-ins, and confidence variants; result: less naming drift and stronger “never miss again” guardrails.
- **UX-37.1 Shipped** — AICommandCenter trade-table + detail-modal inline-style extraction landed in PR #5319: converted the trade-table typography/tags/actions and the trade detail modal metadata/link/button cluster into named styled-components, preserving the existing markup flow while reducing inline-style drift in the primary operator surface.
- **UX-37.2 Shipped** — AICommandCenter confidence-pill + intent-label + record-button extraction landed in PR #5321: finished the remaining `AICommandCenter.tsx` operator-surface cleanup by moving the header search/refresh wrappers, AI Inbox badge/intent/confidence surfaces, pipeline count text, and detail-modal record/open/process-flow wrappers into named styled-components without changing behavior.
- **UX-37.3 Shipped** — Cockpit workspace search/catalog cleanup landed in PR #5324: replaced the Cockpit SmartSearch forwarding stub with static Command Center search guidance and removed `AILearningMetricsWidget` from new widget-catalog additions while preserving render support for existing saved layouts.
- **UX-37.4 Shipped** — Workspace terminology alignment landed in PR #5325: standardized the surviving secondary workspace wording around **Workspace Dashboard** across Header onboarding, Cockpit page titles, tour copy, onboarding empty states, widget CTAs, and cockpit record breadcrumbs without changing routes or tour logic.
- **UX-37.5 Shipped** — AICommandCenter shortcut/deep-link/confidence regression hardening landed in PR #5326: added focused `AICommandCenter.test.tsx` coverage for Alt+1-4 tab switching, input-focus guards, `?item=` modal open/close URL behavior, overview AI Inbox preview limits, and the exact `0.80` / `0.50` / `0.49` confidence boundaries while fixing the draft-review deep-link reopen race those tests exposed.
- **Dependencies:** Phase 36 complete on `development`; no backend/API dependency required for ticket seeding.
- **Risk register + mitigations:**
  - **Styled-component extraction risk (Low × High):** keep conversions additive, preserve existing markup structure where possible, and validate the touched paths with focused Command Center tests before running the full suite.
  - **Cockpit behavior drift risk (Low × Medium):** preserve saved-layout widget rendering, remove only the redundant forwarder/catalog entrypoints, and keep all legacy redirects/routes untouched.
  - **Terminology/test drift risk (Medium × Medium):** update strings and regression coverage in the same ticket so user-facing copy and expectations do not fall out of sync again.
- **Testing strategy:** for docs-only seeding, use `bash scripts/verify_golden_state.sh` and `bash .github/scripts/check_infrastructure.sh`; for each frontend ticket, require `npm -C frontend run verify-standards`, focused `vitest` commands listed in `.github/EPIC_TICKETS.md`, and `npm -C frontend run test:ci`.
...
- **AUTO-38.3 Ready** — Recoverable sync retry/reconnect/progress CTA hardening: add safe reconnect/retry actions and long-sync progress summaries on top of the new error/status contract and the now-shipped provenance/status surfaces.
- **AUTO-38.4 Blocked** — Sample-email dev regression for the automation chain: close the deferred end-to-end coverage gap using deterministic sample-email fixtures across the PO/SO/fulfillment/invoice path after the recoverable retry/progress lane is complete.
...
- **Primary execution focus (P0):** Platform is investor-demo-ready and production-qualified, and the next highest-leverage lane is now **Phase 38 email automation reliability and operator diagnostics**. **AUTO-38.1** and **AUTO-38.2** are now shipped on `development`, so the next concrete task is **AUTO-38.3 recoverable sync retry/reconnect/progress CTA hardening** on top of the newly visible operator status surfaces.
...
- **Quality Audit Hardening: SHIPPED.** Accessibility fixes (ItemCard role/tabIndex/keyboard, aria-labels on ViewProcessFlowButton), theme compliance (replaced hardcoded purples with --color-primary), type safety (eliminated all `any` types in cockpit code), button type="button" additions, and tagRender onClose fix across cockpit components (PR #5072).
- **Final Sprint — Investor Pitch Readiness: SHIPPED.** README.md overhauled (investor-grade feature table, demo access, architecture overview). Created INVESTOR_DEMO_GUIDE.md (7-scene walkthrough + Q&A). Created PRODUCTION_LAUNCH_CHECKLIST.md (infra, security, monitoring, rollback, go/no-go criteria). Backend: admin registrations for deals + integrations (6 models), clarified production CSRF settings. Playwright E2E investor-demo-smoke.spec.ts (7 scenarios). (PR #5073).
- **E2E Test Hardening + Docs Accuracy: SHIPPED.** Hardened Playwright selectors (`:visible`, `domcontentloaded`), HTTP status + error boundary checks, removed overclaim from README, clarified launch checklist promotion workflow (PR #5074).
- **CRITICAL: Nginx Cache Headers Fix: SHIPPED.** Fixed stale Vite chunk 404s crashing dev.meatscentral.com. Added `Cache-Control: no-cache, no-store, must-revalidate` to index.html SPA fallback in all 4 nginx configs (frontend.conf, frontend-ssl.conf, site.conf.template, host-reverse-proxy.conf.template). Hashed assets retain 7-day cache. (PR #5075).
- **AI Widget WebSocket Spam Fix: SHIPPED.** Eliminated reconnection spam on login/page load. Mount-only effect (removed `aiEnabled` dep), connection lock to prevent overlapping connects, 300ms mount debounce, post-async disposed guard, quiet logging (first-occurrence only). Pill text changed to "Inbox offline". (PR #5076).
- **72-Hour Comprehensive Audit Remediation: SHIPPED.** Theme compliance (replaced hardcoded hex colors in ColdStorage with CSS custom properties), button safety (added type="button" to ~20 untyped buttons across 5 files), logging hygiene (replaced print() with logger in entity_introspection), admin registrations for 8 operationally important models: InquirySupplierRFQ, TradeSession, TenantWorkFormExecution, WorkflowDeadLetter, AIFeedbackLog, AIRun, AIApproval, CockpitDraftForm (PR #5078).
- **Performance: UnifiedFlowEditor code splitting: SHIPPED.** Converted all 5 import sites from eager imports to React.lazy() + Suspense boundaries, splitting the 2.1MB editor chunk into a lazy-loaded module that only downloads when users navigate to editor pages. Reduces initial page load by ~600KB+ (PR #5080).
- **Final React #185 eradication + interactive React Flow: SHIPPED.** Stabilized queryKeys in useCustomerProducts (useMemo + ref-based deep-equality guard) and ProcessMonitor (inline object → primitive string). Added onNodeClick prop to UnifiedFlowEditor and wired interactive node clicking in ProcessMonitor and EntityWorkflowStatusPanel — clicking any node now shows details, status, and type inline (PR #5082).
- **Full AI email attachment ingestion pipeline: SHIPPED.** Every email attachment (PDF, Excel/CSV, Word/DOCX, images, plain text) is now downloaded during sync, parsed for text content, and fed into the AI classification engine. New attachment_extractor.py handles all file types using zero new dependencies (stdlib + OpenAI vision for scanned PDFs/images). Extracted text stored in EmailLog.attachment_data JSONField and included in classification prompt for dramatically richer PO/product/contact extraction from attached documents (PR #5084).
- **Attachment pipeline v2 – per-attachment classification + UI display: SHIPPED.** Expanded classification categories (Invoice, Pricing Sheet), per-attachment document type classification (BOL, invoice, PO, pricing sheet, etc.), new extracted fields (po_number, bol_number, total_amount). Attachment-specific lineage events in signals, enriched email logs API with attachment_count/filenames, new Attachments panel in AI Inbox review modal showing per-file doc type tags and extracted key fields (PO#, BOL#, amount). Full frontend + backend integration across 8 files (PR #5086).
- **Dependency-aware entity drafting + unified modal: SHIPPED.** New dependency_drafter.py analyzes email classification to propose related entities (Contact → Supplier → Customer → Inquiry → Invoice/BOL) in correct dependency order with confidence scores. Wired into signal flow post-classification. BatchResolveAPIView for bulk approve operations (max 50). AIDraftReviewModal now shows Related Entity Drafts section with collapsible cards, Approve All / Dismiss All buttons, entity type tags, status badges, and key extracted fields. Frontend aiDraftFormMapping extended for invoice/customer/contact/supplier types. 8 files, 511 insertions (PR #5088).
- **Quality sweep – bug fixes + 100% test pass rate: SHIPPED.** Null-safe TradeLineageFlow (crash on missing current_step), bare except → specific exceptions in core/views.py and email_ingestion.py, 3 test fixes (year-agnostic copyright, CSS variable alignment, missing QueryClientProvider). Added _worktrees/ to .gitignore. All 1453 tests pass, 0 failures (PR #5090).
- **Bulk action toolbar with CSV export: SHIPPED.** Added enableBulkActions prop to UnifiedEntityTable with AntD row selection checkboxes, floating action bar (row count, Export CSV, Clear Selection), client-side CSV generation with proper escaping. Enabled on main entity listings (PR #5092).
- **DRY consolidation + bug fixes: SHIPPED.** Created canonical entityTypeRegistry.ts replacing 4 duplicate entity normalizers. Archived duplicate docs from docs/workforms/. Removed duplicate backend imports. Fixed searchParams mutation bug in Customers.tsx and added missing error handling in handleDelete. 11 files, 358 insertions (PR #5094).
- **Delegation gap closure (11 items): SHIPPED.** SendEmailExecutor wired to contact_resolution service (opt-in resolve_from_contacts). Per-entity Approve/Reject buttons in AIDraftReviewModal. File type emoji icons in EmailIngestionCockpitPanel and AIDraftReviewModal. Activity Feed enriched with attachment count/filenames. Manual WebSocket Reconnect button in AIAgentWidget. Stale process warning bar (>2h) in ProcessFlowHeader. loopDoUntil node type registered. Inline entity creation via MissingDependencyQuickCreate in AIDraftReviewModal. Attachment file count on Process Monitor cards. Complete 30-test matrix in FORM_PROCESS_TESTING_GUIDE.md. 10 files, 325 insertions (PR #5096).
- **CI/CD reliability fixes: SHIPPED.** Fixed 4 root causes for consistently failing PR validation: (1) django-flags DatabaseFlagsSource querying non-existent table on fresh CI DB → set FLAG_SOURCES to settings-only, (2) hardcoded color in ApprovalGateNode.tsx → CSS custom property, (3) trailing whitespace in docs triggering pre-commit failures → stripped, (4) db-sync-prod-to-uat pg_dump version mismatch (PG 17.9 server vs pg_dump 16.x) → install postgresql-client-17 from PGDG repo. 6 files (PR #5098).
- **Holistic audit sweep: SHIPPED.** Critical: db-sync-prod-to-uat.yml wrong table name apps_tenants_tenant → tenants_tenant (4 locations, was silently breaking test-tenant filtering). Unhandled promise rejection in SmartTradeCreator.tsx. Migrated 124 console.error/warn calls to structured logger across 58 files — production telemetry now captures errors that console.* strips in minified builds. 60 files (PR #5104).
- **Complete console→logger migration: SHIPPED.** Final 22 console.error/warn calls in admin-studio (10 files) and Inquiries.tsx. Frontend now has zero console.error/warn in any production code path — all errors route through structured logger for telemetry. 11 files (PR #5106).
- **Pre-commit alignment fix: SHIPPED.** Fixed 3 root causes for PR validation failures: (1) flake8 max-line-length=100 conflicted with Black line-length=120 → aligned to 120, (2) OpenAPI baseline drift — missing contact_routing_details field on PurchaseOrder → regenerated, (3) missing migration 0008_alter_emailreviewdraft_draft_type → generated. Also updated 8 golden state guardrail regex patterns to accept both single/double quotes (Black converts to double). 5 files (PR #5109).
- **Backend test fixes: SHIPPED.** Fixed SalesOrderStatus.COMPLETED → DELIVERED (enum mismatch in happy_path_orchestrator), broadened PO approval dispatch exception catch to prevent 500s, mocked _resolve_recipient for clean test isolation against pre_save signal interference. 4 files (PR #5111).
- **AI Widget WebSocket spam eliminated: SHIPPED.** Added pre-flight HTTP health check before any WebSocket attempt, reduced max retries 8→3, increased backoff base 1s→2s, delayed degraded status display by 5s, removed all debug logging. Permanent fix for browser console spam. 2 files (PR #5112).
- **Delegation gap remediation: SHIPPED.** Comprehensive 13-item audit and fix across all user delegations. Frontend: Process Cockpit added In Progress/AI Inbox/Tasks tabs, ViewProcessFlowButton wired into entity panels, React Flow node approve/reject actions, AIDraftReviewModal top-level batch actions + collapsible sections, DynamicConfigPanel quick-create for entity selects. Backend: per-field confidence scoring in AI classification, clear source tagging in attachment extractor, new dependency-ordered entity creation service, rate limiting (10/m) on email sync tasks, BidSelection enriched contact data, Activity Feed attachment count messages. Also fixed flaky test timeout and pre-existing flake8 violations. 11 files (PR #5114).
- **Frontend standards sweep: SHIPPED.** P0 discovery backlog items resolved: 6 hardcoded `color: white` → `rgb(var(--color-text-inverse))` in PurchaseOrders/Fulfillments/MySubmissions, console.error → structured logger in AdminErrorBoundary + ErrorBoundary, non-semantic div → button in AISuggestionsPanel for keyboard a11y, stale docs/README.md links fixed (dead plan references → canonical MASTER_PLAN.md). 7 files (PR #5116).
- **Comprehensive named color sweep: SHIPPED.** Eliminated all 119 remaining hardcoded named colors (117 `color: white` → `rgb(var(--color-text-inverse))`, 2 `color: black` → `rgb(var(--color-text-primary))`) across 68 frontend files. Zero remaining named color violations in production frontend code. Dark mode and theme switching ready. 68 files (PR #5118).
- **WorkForms keyboard accessibility: SHIPPED.** Dialog semantics (role=dialog, aria-modal, aria-label) on FormSelectorModal and FormPreviewModal overlays. FormSelectorModal form cards: role=option + aria-selected + tabIndex + onKeyDown keyboard handlers. Close buttons: aria-label. Form list: role=listbox. WCAG 2.1 dialog pattern compliant. 2 files (PR #5120).
- **Quality hardening sweep: SHIPPED.** N+1 query elimination: PurchaseOrder ViewSet select_related (7 FKs) + prefetch_related (items), SalesOrder ViewSet select_related (8 FKs). OAuth exception logging: structured logger.warning/exception on broad except blocks. Repo cleanup: removed 58 stale worktrees (13GB recovered), deleted 379 merged branches, archived 3 stale root files. 6 files (PR #5122).
- **Type safety sweep: SHIPPED.** Eliminated all `any` types in 3 high-traffic frontend files: CockpitDashboard (7 → 0, PR #5124), FormSubmissionModal (14 → 0, PR #5126), NodeConfigPanel (11 → 0, PR #5127). Added proper interfaces (SearchEntity, SnapshotStep, FormListItem, SaveResult), `unknown` with type guards, and Array.isArray runtime checks.
- **Backend import cleanup: SHIPPED.** Eliminated 25 unused imports (F401) across 21 production Python files + N+1 query fix for Contact/Carrier ViewSets. Zero remaining F401 in production code. 23 files (PR #5125).
- **Security + N+1 + error info leak fix: SHIPPED.** Removed hardcoded demo credentials from README.md, added select_related to Plant/Location ViewSets, fixed Plant create() leaking `str(e)` to API clients on 500 errors. 3 files (PR #5129).
- **Silent exception swallowing fix: SHIPPED.** Replaced 5 `except Exception: pass` blocks with `logger.warning(exc_info=True)` in views_trades, trade_dependency_checker, and ai_assistant views. Zero silent swallows remain. 3 files (PR #5130).
- **Flake8 F541/F841 production sweep: SHIPPED.** Eliminated all F541 (f-strings without placeholders) and F841 (unused variables) in 12 production files. Zero remaining F541/F841 in production backend code (PR #5131).
- **Management command F541 cleanup: SHIPPED.** Fixed f-strings without placeholders across 12 management commands/utilities + fixed a real bug in seed_system_products dry-run display. 12 files (PR #5132).
- **Undefined name bugs fixed: SHIPPED.** Fixed 3 real runtime bugs — missing `models` import in system admin (F821), missing `Product` import in seed_logistics_data (F821), `action` loop variable shadowing decorator import in workflows views (F402). 3 files (PR #5133).
- **Error info leak sweep: SHIPPED.** Replaced 9 instances of `str(e)` leaked in HTTP 500 API responses with generic error messages + server-side `logger.exception()`. Affected: entity_viewset, admin, monitoring_views, workflows views (cascaded options, form import/duplicate, notification prefs). Also converted silent RLS RESET `pass` to debug-level logging. 5 files (PR #5135).
- **OAuth redirect info leak fix: SHIPPED.** Removed `str(e)` from OAuth callback redirect URLs (browser URL bar exposure). Replaced with generic error codes + server-side `logger.warning()`. Cleaned unused imports. 1 file (PR #5137).
- **Mobile theme token centralization: SHIPPED.** Created `mobile/src/theme/colors.ts` with 30+ semantic color tokens. Replaced ALL hardcoded hex colors across 7 mobile screens. Removed `console.error`/`console.warn` from production mobile code. Zero hardcoded colors and zero console statements remain in mobile. 10 files (PR #5138).
- **Golden state audit + stale docs archive (CICD-22.1, CICD-22.2): SHIPPED.** Golden state verification passes 100%. Archived 14 stale/historical docs (troubleshooting guides, audit snapshots, workforms historical docs, prompt history). Converted duplicate WORKFORMS_USER_GUIDE.md to pointer. Fixed all broken references. All 63 EPIC_TICKETS marked Shipped. 19 files, -459 lines (PR #5140).
- **Dead dependency removal: SHIPPED.** Removed `react-table@7` (v7, zero imports — only `@tanstack/react-table` v8 used), `react-autosuggest`, `react-color`, and their `@types/` packages. 17 total packages removed. Zero TS errors after removal (PRs #5142, #5143).
...
- **Supplier → Plant schema cascade hardening: SHIPPED.** Breadcrumb-derived entity hierarchy is now published through `NavigationContext`, shared create flows auto-cascade and lock parent FK values instead of page-local plant defaults, supplier record/list “New Plant” entrypoints reuse the same modal path, and `Plant.site_code` is now exposed through the additive model/serializer/migration contract. 12 files + 1 migration (PR #5335).
...
- **Email webhooks verification (critical):** Outlook requires unpredictable per-subscription `clientState`; Gmail requires request verification (JWT/secret) so forged requests cannot trigger upstream API calls. ✅ shipped — Outlook `clientState` generated via `secrets.token_urlsafe(32)`, SHA-256 hash stored and verified per-notification; Gmail supports both `GMAIL_PUBSUB_VERIFICATION_TOKEN` and Google-signed OIDC JWT verification; 5 test cases in `test_webhook_verification.py`.
- **Tenant-scope email integration data:** phase in `tenant_id` for EmailAccount/EmailLog (and related tables), then add RLS policies once tenant-scoped. ✅ shipped — all email_integration models (EmailAccount, EmailTrigger, EmailAction, EmailLog) have `tenant` FK; RLS policies added in migrations 0004 + 0005 with safe UUID cast.
