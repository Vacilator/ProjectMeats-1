...
- **Phase 14 execution is advancing:** `GA-01.4 etl-transactional-import-and-reconciliation` now ships the first write-capable, tenant-safe Golden Schema import path for historical purchase orders, sales orders, carrier purchase orders, invoices, and their line items with row-level reconciliation output; the next ready execution ticket is now `GA-02.1 infra-desired-state-and-iac-scaffold`.
