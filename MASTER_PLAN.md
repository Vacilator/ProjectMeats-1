...
**Primary Focus**: Phase 12 execution remains active with `EH-03.1 openapi-ai-and-high-churn-surface-coverage` now promoted as the single top-most ready ticket after `EH-02.3 chat-session-tenant-fk-rls` shipped via PR #4871; Phase 15 B2B Network planning and Phase 16 Core Trading Engine planning remain sealed  
...
- **Primary execution focus (P0):** continue the Phase 12 enterprise hardening lane from the top of `.github/EPIC_TICKETS.md`, starting with `EH-03.1 openapi-ai-and-high-churn-surface-coverage`, now that `EH-02.3 chat-session-tenant-fk-rls` shipped in PR #4871 and removed the last runtime dependency on JSON-stamped tenant context for chat session/message reads.
...
- **Phase 12 - Enterprise Hardening & Tech Debt Eradication (active execution lane)**
  - **Next ready ticket:** `EH-03.1 openapi-ai-and-high-churn-surface-coverage`
  - **Why now:** the chat tenant/RLS foundation is now sealed, so the next remaining high-risk platform gap is missing explicit schema coverage on AI and other high-churn backend surfaces.
  - **Near-term execution ordering:** `EH-03.1`, then continue through the remaining `EH-03.*` through `EH-06.*` foundations in backlog order.
  - **Definition of ready:** `.github/EPIC_TICKETS.md` exposes exactly one first unchecked `Ready` ticket (`EH-03.1`) and keeps later hardening / planning tickets blocked or queued behind it.
