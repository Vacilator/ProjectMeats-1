...
def tenant_owner(django_user_model):
    ...
    return django_user_model.objects.create_user(
        username=f"tenant_owner_{unique}", email=f"tenant-owner-{unique}@example.com", password="testpass123"
    )
...
def tenant_user(django_user_model, test_tenant):
    ...
    user = django_user_model.objects.create_user(
        username=f"tenant_user_{unique}", email=f"tenant-user-{unique}@example.com", password="testpass123"
    )
    TenantUser.objects.create(user=user, tenant=test_tenant, role="admin", is_active=True)
    return user
