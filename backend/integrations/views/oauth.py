...
from apps.integrations.providers.base import AuthenticationError, EmailProviderError
...
class OAuthCallbackView(APIView):
    ...
    def get(self, request, provider: str | None = None, provider_type: str | None = None):
        ...
        if not getattr(rls_result, "ok", False):
            ...
            response = redirect(f"/settings/email-integrations?error=rls_enforcement_failed&provider={quote(provider)}")
            _clear_oauth_cookies(response, provider)
            return response
        ...
        try:
            ...
            response = redirect(f"/settings/email-integrations?error=provider_not_configured&message={quote(str(e))}")
            _clear_oauth_cookies(response, provider)
            return response
