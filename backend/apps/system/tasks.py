...
def execute_workform_execution(self, execution_id: str, tenant_id: str) -> dict:
    ...
    from tenant_apps.workflows.models import TenantWorkFormExecution, TenantWorkFormExecutionStatus, WorkflowDeadLetter
    ...
    try:
        ...
        if any_failed and str(error_strategy) == "stop":
            ...
            if error_target:
                ...
                execution.audit_trail = (result.context or {}).get("audit_trail", [])
                execution.status = (
                    TenantWorkFormExecutionStatus.FAILED
                    if not result.success
                    else TenantWorkFormExecutionStatus.COMPLETED
                )
                execution.error_message = str(result.error or "")
                execution.completed_at = timezone.now()
                execution.save(update_fields=["status", "context_data", "audit_trail", "error_message", "completed_at"])
        ...
        execution.completed_at = timezone.now()
        execution.status = (
            TenantWorkFormExecutionStatus.FAILED if any_failed else TenantWorkFormExecutionStatus.COMPLETED
        )
