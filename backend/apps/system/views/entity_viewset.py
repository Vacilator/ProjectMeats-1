...
class EntityViewSet(viewsets.ViewSet):
    ...
    def relationships(self, request, type=None, pk=None):
        ...
        if type not in self.MODEL_MAP:
            return Response({"error": f"Unknown entity type: {type}"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({"error": f"Model not found: {app_label}.{model_name}"}, status=status.HTTP_404_NOT_FOUND)
        ...
        return Response(
            {"entity": self._serialize_entity(entity, type), "relationships": relationships, "counts": counts}
        )
    ...
    def summary(self, request, type=None, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": f"Failed to generate summary: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def partial_update(self, request, pk=None, type=None):
        ...
        if field_name in {"preferred_products", "active_products"}:
            ...
            if (
                ...
                and hasattr(entity, "products")
                and field_name in {"preferred_products", "active_products"}
            ):
                if not isinstance(raw_value, list):
                    return Response(
                        {"error": "Value must be a list of product IDs"}, status=status.HTTP_400_BAD_REQUEST
                    )
            ...
            return Response(
                {"error": f"Field not editable for {entity_type}: {field_name}"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for field in Model._meta.get_fields():
            ...
            if getattr(field, "is_relation", False) and getattr(field, "many_to_one", False):
                ...
                if rel_obj is not None:
                    ...
                    rel_title = self._serialize_entity(rel_obj, rel_type or field.related_model.__name__.lower()).get(
                        "title"
                    )
    ...
    def _get_contacts_for_customer(self, customer, tenant):
        ...
        try:
            ...
            legacy_ids = set(Contact.objects.filter(tenant=tenant, customer=customer).values_list("id", flat=True))
    ...
    def _get_contacts_for_supplier(self, supplier, tenant):
        ...
        try:
            ...
            legacy_ids = set(Contact.objects.filter(tenant=tenant, supplier=supplier).values_list("id", flat=True))
    ...
    def _get_supplier_for_po(self, purchase_order, tenant):
        ...
        if hasattr(purchase_order, "supplier") and purchase_order.supplier:
            return {"count": 1, "items": [self._serialize_entity(purchase_order.supplier, "supplier")]}
        return None
    ...
    def _serialize_entity(self, entity, entity_type):
        ...
        if entity_type == "contact":
            ...
            base["title"] = getattr(entity, "invoice_number", None) or f"Invoice {entity.pk}"
        elif entity_type == "inquiry":
            base["title"] = getattr(entity, "inquiry_number", None) or f"Inquiry {entity.pk}"
        elif entity_type == "product":
            base["title"] = (
                getattr(entity, "name", None) or getattr(entity, "product_code", None) or f"Product {entity.pk}"
            )
    ...
    def fuzzy_related(self, request, type=None, pk=None):
        ...
        try:
            ...
            fuzzy_matches = discovery_service.discover_related_entities(
                entity_type=type, entity_id=pk, max_results=max_results
            )
            ...
            return Response({"error": f"Entity not found: {type} #{pk}"}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
