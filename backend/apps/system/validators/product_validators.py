...
class ProductValidator:
    ...
    def get_valid_protein_types() -> List[str]:
        ...
        try:
            ...
            valid_items = SystemChoiceItem.objects.filter(
                choice_list=choice_list, is_active=True, tenant__isnull=True  # System-level only
    ...
    def validate_category(value: Optional[str]) -> None:
        ...
        if value not in valid_categories:
            raise ValidationError(f"Invalid category '{value}'. Must be one of: {', '.join(valid_categories)}")
