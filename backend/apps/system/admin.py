...
class SystemChoiceListAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("id", "slug", "name", "description")}),
        ("Configuration", {"fields": ("model_field_path", "is_extensible", "is_reorderable")}),
    ...
    def import_json_view(self, request, pk):
        ...
        try:
            ...
            return JsonResponse({"success": True, "imported": len(items), "created": created_count})
        except SystemChoiceList.DoesNotExist:
            return JsonResponse({"error": "Choice list not found"}, status=404)
            ...
            data.append(
                {
                    ...
                    "name": choice_list.name,
                    "is_extensible": choice_list.is_extensible,
                    "items": [{"value": item.value, "label": item.label, "order": item.order} for item in items],
                }
            )
    ...
    def archive_choice_list(self, request, queryset):
        ...
        self.message_user(request, f"Archived {queryset.count()} choice list(s), deactivated {total_items} items.")
    ...
    def unarchive_choice_list(self, request, queryset):
        ...
        self.message_user(request, f"Unarchived {queryset.count()} choice list(s), activated {total_items} items.")
    ...
    def merge_choice_lists(self, request, queryset):
        ...
        for source in sources:
            ...
            max_order = (
                target.items.filter(tenant__isnull=True).aggregate(max_order=models.Max("order"))["max_order"] or 0
            )
        ...
        self.message_user(
            request, f'Merged {len(sources)} list(s) into "{target.name}". Added {merged_count} unique items.'
        )
...
class SystemChoiceItemAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("id", "choice_list", "tenant")}),
        ("Item Definition", {"fields": ("value", "label", "extra_data")}),
        ("Display", {"fields": ("order", "is_active", "is_default")}),
...
class SystemFieldSchemaAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("id", "field_path", "field_type")}),
        ("Parsed Path", {"fields": ("app_label", "model_name", "field_name"), "classes": ("collapse",)}),
        ("Display", {"fields": ("label", "help_text", "placeholder")}),
        ("Validation", {"fields": ("validation_rules", "default_value")}),
        ("Behavior", {"fields": ("is_required", "is_readonly", "is_hidden")}),
        ("Choice Configuration", {"fields": ("choice_list",), "classes": ("collapse",)}),
        ("Audit", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
...
class TenantConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("id", "tenant", "key", "category")}),
        ("Value", {"fields": ("value", "description")}),
        ("Audit", {"fields": ("created_at", "updated_at", "updated_by"), "classes": ("collapse",)}),
    )
...
class ProductAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identification", {"fields": ("id", "product_code", "name", "description")}),
        ("Classification", {"fields": ("category", "protein_type", "fresh_or_frozen", "edible_or_inedible")}),
        ("Packaging", {"fields": ("package_type", "carton_type", "pcs_per_carton", "unit_weight", "uom")}),
        ("Industry Codes", {"fields": ("namp_code", "usda_code", "ub_code"), "classes": ("collapse",)}),
        ("Attributes", {"fields": ("net_or_catch", "tested_product")}),
        ("Status", {"fields": ("is_active",)}),
        ("Audit", {"fields": ("legacy_tenant_product_id", "created_at", "updated_at"), "classes": ("collapse",)}),
    )
...
class TenantProductPreferenceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Relationships", {"fields": ("id", "tenant", "product")}),
        ("Display Customization", {"fields": ("display_name", "internal_code", "notes")}),
        ("Pricing", {"fields": ("default_price", "default_cost")}),
        ("Supplier", {"fields": ("preferred_supplier", "supplier_item_number")}),
        ("Status", {"fields": ("is_active", "is_favorite", "sort_order")}),
        ("Audit", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
...
class ConfigAuditLogAdmin(admin.ModelAdmin):
    ...
    list_display = ("created_at", "entity_type", "entity_name", "change_type_badge", "user_display", "tenant")
    ...
    fieldsets = (
        ("Entity", {"fields": ("entity_type", "entity_name", "object_id", "content_type")}),
        ("Change Details", {"fields": ("change_type", "field_name", "old_value", "new_value")}),
        ("Snapshots", {"fields": ("snapshot_before", "snapshot_after"), "classes": ("collapse",)}),
        ("User & Tenant", {"fields": ("user", "user_email", "tenant")}),
        ("Request Info", {"fields": ("ip_address", "user_agent", "notes"), "classes": ("collapse",)}),
