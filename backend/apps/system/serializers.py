...
class ChoiceItemCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_value(self, value):
        ...
        if choice_list and tenant:
            existing = SystemChoiceItem.objects.filter(choice_list=choice_list, tenant=tenant, value=value).exists()
            if existing:
                raise serializers.ValidationError(f"Choice item with value '{value}' already exists for this tenant.")
        return value
...
class BulkChoiceUpdateSerializer(serializers.Serializer):
    ...
    items = serializers.ListField(child=serializers.DictField(child=serializers.CharField()))
...
class TenantProductSerializer(serializers.ModelSerializer):
    ...
    unit_weight = serializers.DecimalField(
        source="product.unit_weight", max_digits=10, decimal_places=2, read_only=True
    )
