...
from apps.system.models import FormTypeChoices, TenantForm
from apps.system.services.container_versioning import extract_container_definitions, hash_definition, snapshot_container
...
class ContainerVersioningTests(TestCase):
    ...
    def test_hash_definition_stability(self):
        ...
        definition1 = {"steps": [{"name": "Step 1"}], "container_id": "node-1"}
        definition2 = {"container_id": "node-1", "steps": [{"name": "Step 1"}]}
    ...
    def test_snapshot_container_creates_new(self):
        ...
        container_node = {
            ...
            "data": {"label": "Customer Onboarding", "description": "Onboard new customers"},
        }
        ...
        form = snapshot_container(
            container_node=container_node, child_steps=child_steps, tenant=self.tenant, user=self.user
        )
    ...
    def test_snapshot_container_reuses_existing(self):
        ...
        container_node = {"id": "container-1", "type": "formMultiStepContainer", "data": {"label": "Customer Form"}}

        child_steps = [
            {"id": "step-1", "type": "formStep", "data": {"stepTitle": "Info", "fields": [{"name": "name"}]}}
        ]

        # Create first snapshot
        form1 = snapshot_container(container_node, child_steps, self.tenant, self.user)

        # Create second identical snapshot
        form2 = snapshot_container(container_node, child_steps, self.tenant, self.user)
