...
class WorkFormExecutionAuditTrailTests(TestCase):
    ...
    def test_execute_task_persists_audit_trail(self):
        ...
        persisted = list(
            ExecutionEventLog.objects.filter(workform_execution=execution)
            .order_by("sequence")
    ...
    def test_execute_task_persists_successful_action_event_logs(self):
        ...
        success_event = ExecutionEventLog.objects.get(
            workform_execution=execution, event_type="action_success", node_id="a1"
        )
    ...
    def test_execute_task_persists_failed_action_event_logs(self):
        ...
        failure_event = ExecutionEventLog.objects.get(
            workform_execution=execution, event_type="action_error", node_id="a1"
        )
