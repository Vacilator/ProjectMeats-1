...
from apps.system.models import SystemChoiceItem, SystemChoiceList, TenantChoiceOverride
...
class ChoiceEngineService:
    ...
    def get_choices_for_field(
        self, slug: str, include_inactive: bool = False, workform_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if self.tenant:
            ...
            items_qs = items_qs.filter(Q(tenant__isnull=True) | Q(tenant=self.tenant))

            # Apply tenant overrides (disabled items)
            try:
                override = TenantChoiceOverride.objects.get(tenant=self.tenant, choice_list=choice_list)
    ...
    def add_tenant_custom_choice(
        self, slug: str, value: str, label: str, order: int = 100, extra_data: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        exists = SystemChoiceItem.objects.filter(choice_list=choice_list, tenant=self.tenant, value=value).exists()
    ...
    def disable_system_choice(self, slug: str, item_id: str) -> None:
        ...
        override, _ = TenantChoiceOverride.objects.get_or_create(tenant=self.tenant, choice_list=choice_list)
