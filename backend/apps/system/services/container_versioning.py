...
def get_next_version(tenant, source_node_id: str) -> int:
    ...
    Returns:
        Next version number
    """
    latest = TenantForm.objects.filter(tenant=tenant, source_node_id=source_node_id).order_by("-version").first()
...
def has_container_changed(container_node: dict, existing_form: TenantForm) -> tuple[bool, dict]:
    ...
    label_candidate = container_data.get("containerName") or container_data.get("label") or container_data.get("name")
...
@transaction.atomic
def snapshot_container(container_node: dict, child_steps: list[dict], tenant, user=None) -> TenantForm:
    """
    ...
    container_description = container_data.get("containerDescription") or container_data.get("description") or ""
    ...
    existing = TenantForm.objects.filter(tenant=tenant, definition_hash=definition_hash).first()
