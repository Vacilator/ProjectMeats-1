...
class RelationshipDiscoveryService:
    ...
    def discover_related_entities(
        self, entity_type: str, entity_id: int, max_results: int = 50
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _find_by_email_domain(self, customer, max_results: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            emails = EmailLog.objects.filter(tenant=self.tenant, sender__icontains=f"@{domain}")[:max_results]
        ...
        try:
            ...
            orders = Order.objects.filter(tenant=self.tenant, customer_email__icontains=f"@{domain}")[:max_results]
                ...
                results.append(
                    {
                        ...
                        "metadata": {"match_type": "email_domain", "domain": domain},
    ...
    def _find_by_tax_id(self, customer, max_results: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            invoices = Invoice.objects.filter(tenant=self.tenant, customer_tax_id=tax_id_clean)[:max_results]
                ...
                results.append(
                    {
                        ...
                        "metadata": {"match_type": "tax_id", "tax_id": tax_id_clean},
    ...
    def _find_by_name(self, entity, max_results: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            Order = apps.get_model("orders", "Order")
            orders = Order.objects.filter(tenant=self.tenant, customer_name__icontains=name_clean)[:max_results]
                ...
                results.append(
                    {
                        ...
                        "metadata": {"match_type": "name", "name": name_clean},
    ...
    def _find_by_sku(self, product, max_results: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            line_items = OrderLineItem.objects.filter(tenant=self.tenant, product_sku=sku_clean).select_related(
                "order"
            )[:max_results]
                ...
                results.append(
                    {
                        ...
                        "type": "order",
                        "name": f"Order #{item.order.order_number}"
                        if hasattr(item.order, "order_number")
                        else f"Order #{item.order.id}",
                        "subtitle": f"SKU: {sku_clean}",
                        "metadata": {"match_type": "sku", "sku": sku_clean},
                        "relevance_score": 0.9,
                    }
                )
        except LookupError:
    ...
    def _find_by_reference_number(self, order, max_results: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            Invoice = apps.get_model("invoices", "Invoice")
            invoices = Invoice.objects.filter(tenant=self.tenant, order_reference=ref_clean)[:max_results]
                ...
                results.append(
                    {
                        ...
                        "metadata": {"match_type": "reference_number", "reference": ref_clean},
