...
class SystemChoiceList(models.Model):
    ...
    name = models.CharField(max_length=255, help_text="Human-readable name (e.g., 'Protein Type', 'Payment Terms')")
    description = models.TextField(blank=True, help_text="Optional description of what this choice list is used for")

    # Where this choice list is used (for documentation and UI hints)
    model_field_path = models.CharField(
        max_length=255, blank=True, help_text="Dot-path to model field (e.g., 'products.Product.protein_type')"
    )

    # Behavioral flags
    is_extensible = models.BooleanField(default=True, help_text="Whether tenant admins can add custom items")
    is_reorderable = models.BooleanField(default=True, help_text="Whether items can be reordered")

    # Audit fields
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    created_by = models.ForeignKey(
        "auth.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="created_choice_lists"
    )
...
class SystemChoiceItem(models.Model):
    ...
    value = models.CharField(max_length=100, help_text="Value stored in database (e.g., 'BEEF', 'NET30')")
    label = models.CharField(max_length=255, help_text="Human-readable label (e.g., 'Beef', 'Net 30 Days')")

    # Optional extra data for complex choices
    extra_data = models.JSONField(
        default=dict, blank=True, help_text="Additional metadata (e.g., icon, color, description)"
    )

    # Ordering and visibility
    order = models.PositiveIntegerField(default=0, help_text="Sort order within the list (lower = first)")
    is_active = models.BooleanField(default=True, help_text="Whether this item is available for selection")
    is_default = models.BooleanField(default=False, help_text="Whether this is the default selection")
