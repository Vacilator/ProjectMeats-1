...
class TenantChoiceOverride(models.Model):
    ...
    disabled_system_items = ArrayField(
        models.UUIDField(), default=list, blank=True, help_text="IDs of system items to hide from this tenant"
    )

    # Custom display configuration
    display_config = models.JSONField(
        default=dict, blank=True, help_text="Custom display preferences (e.g., custom ordering, grouping)"
    )
    ...
    def get_visible_system_items(self):
        ...
        system_items = SystemChoiceItem.objects.filter(
            choice_list=self.choice_list, tenant__isnull=True, is_active=True
        )
    ...
    def get_all_items_for_tenant(self):
        ...
        items = (
            SystemChoiceItem.objects.filter(choice_list=self.choice_list, is_active=True)
            .filter(query)
