...
class TenantWorkForm(models.Model):
    ...
    # WorkForm metadata
    name = models.CharField(max_length=255, help_text="Workflow name/title")
    ...
    #   }
    # }
    workflow_definition = models.JSONField(default=dict, help_text="Complete React Flow canvas state (nodes + edges)")
    ...
    form_references = ArrayField(
        models.UUIDField(), default=list, blank=True, help_text="List of TenantForm IDs referenced in this workflow"
    )

    # Versioning
    version = models.IntegerField(default=1, help_text="Workflow version number (auto-incremented on update)")
    ...
    last_executed_at = models.DateTimeField(null=True, blank=True, help_text="When this workflow was last executed")
    ...
    clone_count = models.IntegerField(default=0, help_text="Number of times this workflow has been cloned")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["tenant", "name", "version"], name="unique_tenant_workform_name_version")
        ]
    ...
    def validate_form_references(self):
        ...
        existing_forms = TenantForm.objects.filter(tenant=self.tenant, id__in=form_ids).values_list("id", flat=True)

        missing_forms = [fid for fid in form_ids if fid not in existing_forms]

        return {"valid": len(missing_forms) == 0, "missing_forms": missing_forms, "total_references": len(form_ids)}
    ...
    def validate_runtime_support(self) -> dict:
        ...
        if not isinstance(nodes, list):
            return {
                ...
                "unsupported_actions": [
                    {"node_id": None, "node_type": "invalid_definition", "label": "Workflow definition is malformed"}
                ],
                "warnings": [],
            }
        ...
        for node in nodes:
            ...
            if (
                ...
                and node_type.startswith("action")
                and node_type not in supported_action_node_types
            ):
            ...
            if node_type in {
                ...
                warnings.append(
                    {"node_id": node_id, "node_type": node_type, "label": label, "warning": "form_nodes_not_executed"}
                )
    ...
    def get_container_nodes(self):
        ...
        return [node for node in nodes if isinstance(node, dict) and node.get("type") in container_types]
