...
class ConfigAuditLog(models.Model):
    ...
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, help_text="Type of entity that was changed")
    object_id = models.CharField(max_length=255, help_text="ID of the changed entity")
    content_object = GenericForeignKey("content_type", "object_id")

    # Human-readable reference
    entity_type = models.CharField(
        max_length=50, help_text="Human-readable entity type (e.g., 'TenantConfig', 'ChoiceList')"
    )
    entity_name = models.CharField(max_length=255, help_text="Name/key of the entity for display")

    # Change details
    change_type = models.CharField(max_length=10, choices=ChangeType.choices, help_text="Type of change")
    field_name = models.CharField(max_length=100, blank=True, help_text="Specific field that was changed (for updates)")
    old_value = models.JSONField(null=True, blank=True, help_text="Previous value (JSON)")
    new_value = models.JSONField(null=True, blank=True, help_text="New value (JSON)")

    # Full entity snapshot for complex changes
    snapshot_before = models.JSONField(null=True, blank=True, help_text="Complete entity state before change")
    snapshot_after = models.JSONField(null=True, blank=True, help_text="Complete entity state after change")
    ...
    user_email = models.EmailField(blank=True, help_text="Email snapshot for display even if user deleted")
