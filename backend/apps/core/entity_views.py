...
def entity_schema(request, entity_type):
    ...
    if entity_type not in entity_map:
        return Response({"error": f"Entity type '{entity_type}' not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    try:
        ...
        return Response({"error": f"Model {app_label}.{model_name} not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    return Response({"entity_type": entity_type, "app_label": app_label, "model_name": model_name, "fields": fields})
