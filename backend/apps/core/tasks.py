...
def cache_workflow_data(tenant_id: str, workflow_id: str) -> Dict[str, Any]:
    ...
    def fetch_workflow():
        ...
        with tenant_rls(str(tenant_id), strict=True):
            workflow = TenantForm.objects.get(id=workflow_id, tenant_id=tenant_id)
            return {"id": str(workflow.id), "name": workflow.name, "fields_count": workflow.fields.count()}
...
def parallel_tenant_sync(tenant_ids: List[str]) -> List[Dict[str, Any]]:
    ...
    """
    tasks = [lambda tid=tid: sync_single_tenant(tid) for tid in tenant_ids]
...
def sync_single_tenant(tenant_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"tenant_id": tenant_id, "status": "success", "workflow_count": workflow_count, "user_count": user_count}
    except Exception as e:
        return {"tenant_id": tenant_id, "status": "error", "error": str(e)}


@shared_task
def batch_export_workflows(tenant_id: str, workflow_ids: List[str]) -> Dict[str, Any]:
    """
    ...
    job = group([export_single_workflow.s(tenant_id, wid) for wid in workflow_ids])

    result = job.apply_async()

    return {"job_id": result.id, "workflow_count": len(workflow_ids), "status": "processing"}
...
def export_single_workflow(tenant_id: str, workflow_id: str) -> Dict[str, Any]:
    ...
    try:
        with tenant_rls(str(tenant_id), strict=True):
            ...
            return {"workflow_id": workflow_id, "status": "success", "data": serializer.data}
    except Exception as e:
...
def aggregate_tenant_metrics(tenant_id: str) -> Dict[str, Any]:
    ...
    count_tasks = [count_workflows_by_status.s(tenant_id, status) for status in ["draft", "active", "archived"]]
    ...
    return {"job_id": job.id, "tenant_id": tenant_id, "status": "processing"}
...
def count_workflows_by_status(tenant_id: str, status: str) -> Dict[str, int]:
    ...
    with tenant_rls(str(tenant_id), strict=True):
        count = TenantForm.objects.filter(tenant_id=tenant_id, status=status).count()
...
def aggregate_counts(results: List[Dict[str, int]], tenant_id: str) -> Dict[str, Any]:
    ...
    CacheService.cache_query_result(cache_key, lambda: aggregated, ttl=300)  # 5 minutes

    return {"tenant_id": tenant_id, "metrics": aggregated, "cached": True}
