...
def guest_login(request):
    ...
    try:
        ...
        if not guest_user.is_active:
            return Response(
                {"error": "Guest account is currently disabled"}, status=status.HTTP_503_SERVICE_UNAVAILABLE
            )
        ...
        guest_tenant_user = TenantUser.objects.filter(user=guest_user, is_active=True).select_related("tenant").first()
    ...
    def get(self, request):
        ...
        if choice_type:
            ...
            if choice_type not in self.CHOICE_CLASSES:
                return Response({"error": f"Unknown choice type: {choice_type}"}, status=status.HTTP_404_NOT_FOUND)

            choice_class = self.CHOICE_CLASSES[choice_type]
            options = [{"value": c.value, "label": c.label} for c in choice_class]
        ...
        for choice_name, choice_class in self.CHOICE_CLASSES.items():
            all_choices[choice_name] = [{"value": c.value, "label": c.label} for c in choice_class]
...
class RankedSearchView(APIView):
    ...
    def get(self, request):
        ...
        if len(query) < 2:
            return Response(
                {"query": query, "results": [], "total": 0, "message": "Query must be at least 2 characters"}
            )
        ...
        return Response(
            {
                ...
                "applied_filters": {"date_range": date_range, "entity_types": entity_types or "all"},
            }
        )
    ...
    def _enhance_result(self, result: dict, query: str, tenant) -> dict:
        ...
        """
        from apps.system.services.ranking_service import EntityLabels, EntityRanking
        ...
        if not entity_obj:
            ...
            score = EntityRanking.calculate_total_score(entity_type=entity_type, entity=entity_obj, query=query)
...
class RecentItemsView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"items": recent, "count": len(recent)})
...
class EntityRelationshipsView(APIView):
    ...
    def get(self, request, entity_type: str, entity_id: int, relationship_name: str = None):
        ...
        if relationship_name:
            ...
            result = service.get_related_entities(entity_type, entity_id, relationship_name, limit=limit, offset=offset)
            return Response(result)
        else:
            # Get all relationships summary
            include_counts = request.query_params.get("counts", "true").lower() == "true"
            relationships = service.get_relationships(entity_type, entity_id, include_counts=include_counts)
            return Response({"entity_type": entity_type, "entity_id": entity_id, "relationships": relationships})
...
class EntityGraphView(APIView):
    ...
    def get(self, request, entity_type: str, entity_id: int):
        ...
        service = EntityGraphService(tenant=request.tenant)
        graph = service.get_entity_graph(entity_type, entity_id, depth=depth, max_nodes=max_nodes)

        if not graph["nodes"]:
            return Response({"error": "Entity not found or no relationships"}, status=status.HTTP_404_NOT_FOUND)
...
class EntityTypesView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"entity_types": types, "visuals": visuals})
...
class WorkspaceLayoutView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"layout": None, "widgets": None, "version": 1})
...
class WorkspaceStatsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {
                    "stats": [
                        ...
                        },
                        {
                            ...
                            "value": PurchaseOrder.objects.filter(tenant=tenant, status="pending").count(),
                            "change": 0,
            ...
            return Response({"stats": [], "error": str(e)})
...
class WorkspaceActivityView(APIView):
    ...
    def _parse_sources(self, request):
        ...
        requested = {str(value).strip().lower() for value in raw_sources.split(",") if str(value).strip()}
    ...
    def _build_ai_items(self, tenant, entity_type, entity_id, start_at, end_at, limit):
        ...
        if entity_type:
            queryset = queryset.filter(Q(target_type__iexact=entity_type) | Q(source_type__iexact=entity_type))
        if entity_id:
    ...
    def get(self, request):
        ...
        if "workflow" in sources:
            activities.extend(
                self._build_workflow_items(request.tenant, entity_type, entity_id, start_at, end_at, limit)
            )
        if "note" in sources:
