...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'app_label',
            type=str,
            help='App label (e.g., "workflows")'
        )
        parser.add_argument(
            'migration_names',
            nargs='+',
            type=str,
            help='Migration name prefixes to delete (e.g., "0012" "0013")'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            for prefix in migration_prefixes:
                ...
                cursor.execute(
                    "SELECT id, name, applied FROM django_migrations "
                    "WHERE app = %s AND name LIKE %s",
                    [app_label, f"{prefix}%"]
                )
                ...
                if not rows:
                    self.stdout.write(self.style.WARNING(
                        f"  No migrations found for {app_label}.{prefix}%"
                    ))
                    continue
                ...
                for row in rows:
                    ...
                    self.stdout.write(
                        f"  Found: {app_label}.{name} (ID: {migration_id}, Applied: {applied})"
                    )
                ...
                if not dry_run:
                    ...
                    self.stdout.write(self.style.SUCCESS(
                        f"  ✓ Deleted {deleted_count} record(s) for {app_label}.{prefix}%"
                    ))
                else:
                    self.stdout.write(self.style.WARNING(
                        f"  [DRY RUN] Would delete {len(rows)} record(s)"
                    ))
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.WARNING(
                "Run 'python manage.py migrate' to apply corrected migrations."
            ))
