...
from tenant_apps.invoices.models import Claim, Invoice
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not user:
            user = User.objects.create_user(
                username="admin", email="admin@meatscentral.com", password="admin123", is_staff=True, is_superuser=True
            )
    ...
    def seed_products(self, tenant, count=8):
        ...
        """
        self.stdout.write(
            self.style.WARNING("⚠️  Skipping product seeding: run `python manage.py seed_system_products` instead.")
        )
        ...
        for po in pos:
            if po.payment_status in ["paid", "partial"]:
                ...
                PaymentTransaction.objects.create(
                    ...
                    else f"WIRE-{random.randint(1000, 9999)}",
