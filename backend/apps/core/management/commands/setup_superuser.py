...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if conflicting_username and conflicting_email and conflicting_username.id == conflicting_email.id:
            ...
            self.stdout.write(self.style.ERROR(f"❌ CONFLICT: Username and email belong to different users!"))
        ...
        if user_existed:
            ...
            if not user.is_superuser:
                ...
                self.stdout.write(self.style.WARNING(f"⚠️  User exists but is not superuser. Promoting to superuser."))
            ...
            self.stdout.write(self.style.SUCCESS(f"🎉 Superuser ready to use!"))
            self.stdout.write(f"   Username: {username}")
            self.stdout.write(f"   Email: {email}")
