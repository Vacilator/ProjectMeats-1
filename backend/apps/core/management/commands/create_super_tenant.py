        ...
        try:
            ...
            with transaction.atomic():
                ...
                if users_by_username.count() > 1:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            ...
                            "Using first user and deleting duplicates."
                        )
                    )
                ...
                if user_created:
                    ...
                    if verbosity >= 2:
                        self.stdout.write("   - Updated password for existing superuser")
                ...
                if link_created:
                    ...
                    self.stdout.write(self.style.SUCCESS("✅ Superuser linked to root tenant as owner"))
                else:
                    if verbosity >= 2:
                        self.stdout.write(f"   - Link already exists: user {user.username} -> tenant {tenant.slug}")
                    self.stdout.write(self.style.WARNING("⚠️  Superuser already linked to root tenant"))
