...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--fix", action="store_true", help="Automatically create missing RLS policies")
        parser.add_argument("--strict", action="store_true", help="Exit non-zero if any models are not RLS compliant")
    ...
    def handle(self, *args, **options):
        ...
        for result in results:
            if result["compliant"]:
                self.stdout.write(self.style.SUCCESS(f"✓ {result['model']} ({result['table']})"))
                compliant_count += 1
            else:
                self.stdout.write(self.style.ERROR(f"✗ {result['model']} ({result['table']})"))

                if not result["rls_enabled"]:
                    self.stdout.write("  → RLS not enabled")
        ...
        if compliant_count == len(results):
            ...
            self.stdout.write(self.style.ERROR(f"✗ {len(results) - compliant_count} models need attention"))
            if not options["fix"]:
                self.stdout.write("")
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if not result["rls_enabled"]:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"  ✓ Enabled RLS on {table_name}"))
                ...
                if not result["policy_exists"]:
                    ...
                    """)
                    self.stdout.write(self.style.SUCCESS(f"  ✓ Created policy {policy_name}"))
