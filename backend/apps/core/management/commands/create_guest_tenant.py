...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--username", type=str, default="guest", help="Username for the guest user (default: guest)"
        )
        parser.add_argument(
            "--password", type=str, default="guest123", help="Password for the guest user (default: guest123)"
        )
        parser.add_argument(
            "--tenant-name",
            type=str,
        ...
        parser.add_argument(
            "--tenant-slug", type=str, default="guest-demo", help="Slug for the guest tenant (default: guest-demo)"
        )
        ...
        try:
            with transaction.atomic():
                ...
                if user_created:
                    ...
                    guest_user.save()
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Created guest user: {username} (staff=True, superuser=False)")
                    )
                else:
                    ...
                    if not guest_user.is_staff:
                        ...
                        guest_user.save()
                        self.stdout.write(self.style.SUCCESS(f"✓ Updated guest user to staff: {username}"))
                    self.stdout.write(
                        self.style.WARNING(
                ...
                if tenant_created:
                    self.stdout.write(self.style.SUCCESS(f"✓ Created guest tenant: {tenant_name} ({tenant_slug})"))
                else:
                    self.stdout.write(self.style.WARNING(f"⚠ Guest tenant already exists: {tenant_name}"))
                ...
                if tu_created:
                    self.stdout.write(self.style.SUCCESS(f"✓ Associated {username} with {tenant_name} (role: admin)"))
                else:
                    self.stdout.write(
                        self.style.WARNING(f"⚠ Guest user already associated with tenant (role: {tenant_user.role})")
                    )
                ...
                self.stdout.write("Role: admin (tenant-level)")
                self.stdout.write("Staff: Yes (can access Django admin for testing)")
                self.stdout.write("Superuser: No (no system-wide access)")
                self.stdout.write("")
            ...
            self.stdout.write(self.style.ERROR(f"✗ Error creating guest setup: {str(e)}"))
            raise
    ...
    def _grant_permissions(self, user):
        ...
        self.stdout.write(self.style.SUCCESS(f"✓ Granted {permissions_granted} permissions for tenant-scoped models"))
