...
class Command(BaseCommand):
    ...
    def generate_python_sbom(self, output_dir):
        ...
        if not requirements_file.exists():
            self.stdout.write(
                self.style.ERROR(f'  ✗ requirements.txt not found: {requirements_file}')
            )
            return False
        ...
        try:
            ...
            if result.returncode != 0:
                self.stdout.write(
                    self.style.WARNING('  ! Installing cyclonedx-bom...')
                )
                subprocess.run(
            ...
            if result.returncode == 0:
                self.stdout.write(
                    self.style.SUCCESS(f'  ✓ Python SBOM: {output_file}')
                )
                ...
                self.stdout.write(
                    self.style.ERROR(f'  ✗ Failed: {result.stderr}')
                )
                return False
    ...
    def generate_javascript_sbom(self, output_dir):
        ...
        if not package_json.exists():
            self.stdout.write(
                self.style.ERROR(f'  ✗ package.json not found: {package_json}')
            )
            return False
        ...
        try:
            ...
            if result.returncode == 0:
                self.stdout.write(
                    self.style.SUCCESS(f'  ✓ JavaScript SBOM: {output_file}')
                )
    ...
    def generate_combined_report(self, output_dir):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'  ✓ Summary report: {report_file}')
            )
        
        except Exception as e:
            self.stdout.write(
                self.style.WARNING(f'  ! Could not generate summary: {str(e)}')
            )
