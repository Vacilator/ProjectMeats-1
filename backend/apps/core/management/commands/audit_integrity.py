        ...
        with connection.cursor() as cursor:
            for model in tenant_models:
                ...
                if not policies:
                    ...
                    self.stdout.write(self.style.ERROR(
                        f"  ❌ {model._meta.app_label}.{model.__name__}: NO RLS POLICY"
                    ))
                elif self.verbose:
                    self.stdout.write(self.style.SUCCESS(
    ...
    def _check_viewset_filters(self, report: Dict[str, Any]):
        ...
        for file_path in viewset_files:
            ...
            if 'ViewSet' in content or 'APIView' in content:
                ...
                has_tenant_filter = (
                    'tenant=request.tenant' in content or
                    'tenant=self.request.tenant' in content or
                    'TenantAwareViewSetMixin' in content or
                    'filter(tenant=' in content
                )
                ...
                if not has_tenant_filter:
                    ...
                    self.stdout.write(self.style.WARNING(
                        f"  ⚠️  {file_path.relative_to(settings.BASE_DIR)}: No tenant filter found"
                    ))
                elif self.verbose:
                    self.stdout.write(self.style.SUCCESS(
                        f"  ✓ {file_path.relative_to(settings.BASE_DIR)}"
                    ))
