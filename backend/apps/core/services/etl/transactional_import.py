...
def _prepare_row(loaded_row: LoadedSourceRow, *, tenant, actor, normalized_payload: dict[str, Any]) -> PreparedRow:
    ...
    try:
        ...
        raise RowPreparationError('unsupported_entity', f"Unsupported transactional entity '{loaded_row.entity}'.") from exc
...
def _update_instance(prepared: PreparedRow):
    ...
    if changed_fields:
        instance.save(update_fields=changed_fields + ['updated_at'] if hasattr(instance, 'updated_at') else changed_fields)
    return instance
...
def _prepare_purchase_order(payload: dict[str, Any], *, tenant, actor) -> PreparedRow:
    ...
    defaults = {
        ...
        'our_purchase_order_number_to_supplier': _value(payload, 'our_purchase_order_number_to_supplier') or order_number,
...
def _prepare_purchase_order_item(payload: dict[str, Any], *, tenant, actor) -> PreparedRow:
    ...
    matched = PurchaseOrderItem.objects.filter(
        tenant=tenant,
        purchase_order=purchase_order,
        line_number=line_number,
    ).order_by('pk').first()
...
def _prepare_sales_order(payload: dict[str, Any], *, tenant, actor) -> PreparedRow:
    sales_order_number = _required(payload, 'our_sales_order_number_for_customer', 'our_sales_order_num', 'sales_order_number')
    supplier = _resolve_supplier(payload, tenant=tenant)
    ...
    defaults = {
        ...
        'our_sales_order_number_for_customer': _value(payload, 'our_sales_order_number_for_customer') or sales_order_number,
...
def _prepare_sales_order_item(payload: dict[str, Any], *, tenant, actor) -> PreparedRow:
    ...
    matched = SalesOrderItem.objects.filter(
        tenant=tenant,
        sales_order=sales_order,
        line_number=line_number,
    ).order_by('pk').first()
...
def _prepare_carrier_purchase_order(payload: dict[str, Any], *, tenant, actor) -> PreparedRow:
    ...
    supplier = _resolve_supplier(payload, tenant=tenant)
    linked_order = _resolve_purchase_order(payload, tenant=tenant, required=False, names=('linked_order_number', 'purchase_order_number'))
    ...
    matched = CarrierPurchaseOrder.objects.filter(tenant=tenant, our_carrier_po_num=carrier_po_number).order_by('pk').first()
...
def _prepare_carrier_po_item(payload: dict[str, Any], *, tenant, actor) -> PreparedRow:
    ...
    matched = CarrierPOItem.objects.filter(
        tenant=tenant,
        carrier_purchase_order=carrier_purchase_order,
        line_number=line_number,
    ).order_by('pk').first()
...
def _prepare_invoice(payload: dict[str, Any], *, tenant, actor) -> PreparedRow:
    ...
    defaults = {
        ...
        'our_sales_order_number_for_customer': _value(payload, 'our_sales_order_number_for_customer', 'our_sales_order_num'),
        'delivery_po_number': _value(payload, 'delivery_po_number', 'delivery_po_num'),
        'payment_terms': _normalize_optional_choice(
            payload.get('payment_terms'),
...
def _prepare_invoice_item(payload: dict[str, Any], *, tenant, actor) -> PreparedRow:
    ...
    matched = InvoiceItem.objects.filter(
        tenant=tenant,
        invoice=invoice,
        line_number=line_number,
    ).order_by('pk').first()
...
def _line_item_defaults(payload: dict[str, Any], *, model_class) -> dict[str, Any]:
    return {
        ...
        'product_description': _resolve_product(payload, required=False, code_fields=('product_code', 'product_description_code')),
        'fresh_or_frozen': _normalize_optional_choice(
            payload.get('fresh_or_frozen'),
...
def _resolve_sales_order(
    ...
    *,
    tenant,
    required: bool = True,
    names: tuple[str, ...] = ('sales_order_number', 'sales_order', 'our_sales_order_number_for_customer', 'our_sales_order_num'),
):
    ...
    if not sales_order_number:
        ...
        return None
    sales_order = SalesOrder.objects.filter(tenant=tenant, our_sales_order_num=sales_order_number).order_by('pk').first()
...
def _resolve_carrier_purchase_order(payload: dict[str, Any], *, tenant, required: bool = True):
    ...
    if not carrier_po_number:
        ...
        return None
    carrier_purchase_order = CarrierPurchaseOrder.objects.filter(
        tenant=tenant,
        our_carrier_po_num=carrier_po_number,
    ).order_by('pk').first()
