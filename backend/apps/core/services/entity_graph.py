...
class EntityGraphService:
    ...
    def get_relationships(self, entity_type: str, entity_id: int, include_counts: bool = True) -> List[Dict[str, Any]]:
        """
            ...
            for rel_name, rel_config in config.get("relationships", {}).items():
                try:
                    ...
                    if rel_config.get("computed"):
                        ...
                        related_type = rel_config.get("entity_type") or (
                            "product" if rel_name == "related_products" else rel_name.rstrip("s")
                        )
                    else:
                        if not RelatedModel:
                            continue
                    ...
                    logger.warning(f"Error computing relationship '{rel_name}' for {entity_type}/{entity_id}: {e}")
                    continue
    ...
    def get_related_entities(
        self, entity_type: str, entity_id: int, relationship_name: str, limit: int = 20, offset: int = 0
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if rel_config.get("computed"):
                related_qs = self._get_computed_relationship_qs(rel_config["computed"], obj)
                related_type = rel_config.get("entity_type") or (
                    "product" if relationship_name == "related_products" else relationship_name.rstrip("s")
                )
            else:
                if not RelatedModel:
                    return {"items": [], "total": 0}
    ...
    def get_entity_graph(self, entity_type: str, entity_id: int, depth: int = 1, max_nodes: int = 50) -> Dict[str, Any]:
        """
