...
class UserPreferencesSerializer(serializers.ModelSerializer):
    ...
    def _normalize_tour_statuses(value, *, strict):
        ...
        for raw_tour_name, raw_status in value.items():
            if not isinstance(raw_tour_name, str):
                if strict:
                    raise serializers.ValidationError({"tour_statuses": "Tour names must be strings."})
                continue
            ...
            if not isinstance(raw_status, dict):
                if strict:
                    raise serializers.ValidationError({"tour_statuses": "Each tour status must be an object."})
                continue
    ...
    def _normalize_onboarding_state(value):
        ...
        if unsupported_keys:
            ...
            raise serializers.ValidationError(f"Unsupported onboarding state keys: {unsupported}.")

        completed_tours = value.get("completed_tours", [])
        if not isinstance(completed_tours, list):
            raise serializers.ValidationError({"completed_tours": "Completed tours must be a list of tour names."})

        normalized_tours = []
        seen = set()
        for tour_name in completed_tours:
            if not isinstance(tour_name, str):
                raise serializers.ValidationError({"completed_tours": "Completed tours must contain only strings."})
...
class PortalGrantSerializer(serializers.ModelSerializer):
    ...
    def validate_resource_scope(self, value):
        ...
        if unsupported:
            raise serializers.ValidationError(f"Unsupported portal entity scopes: {', '.join(unsupported)}.")
        ...
        for entity_type, record_ids in value.items():
            ...
            for record_id in record_ids:
                if not isinstance(record_id, str):
                    raise serializers.ValidationError(f"Resource scope ids for {entity_type} must be strings.")
    ...
    def validate_document_sources(self, value):
        ...
        if unsupported:
            raise serializers.ValidationError(f"Unsupported portal document sources: {', '.join(sorted(unsupported))}.")
...
class PortalGrantOperatorSummarySerializer(serializers.ModelSerializer):
    ...
    def get_documents(self, obj):
        ...
        if links is None:
            links = obj.document_links.select_related("document_reference").order_by("sort_order", "created_on")

        documents = [
            link.document_reference
            for link in links
            if link.document_reference.is_active and link.document_reference.source_kind in obj.document_sources
        ]
