...
class UserPreferencesModelTest(TestCase):
    ...
    def test_create_user_preferences(self):
        ...
        preferences = UserPreferences.objects.create(user=self.user, theme="dark", sidebar_collapsed=True)
...
class UserPreferencesSerializerTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="serializer-user", email="serializer@example.com", password="testpass123"
        )
    ...
    def test_serializer_exposes_default_onboarding_state(self):
        ...
        self.assertEqual(serializer.data["onboarding_state"], {"completed_tours": [], "tour_statuses": {}})

    def test_serializer_merges_onboarding_state_without_clobbering_widget_preferences(self):
        preferences = UserPreferences.objects.create(user=self.user, widget_preferences={"sales": {"collapsed": False}})
    ...
    def test_serializer_rejects_invalid_onboarding_state(self):
        ...
        serializer = UserPreferencesSerializer(
            preferences, data={"onboarding_state": {"completed_tours": "cockpit"}}, partial=True
        )
...
class UserPreferencesAPITest(APITestCase):
    ...
    def test_get_or_create_preferences(self):
        ...
        self.assertTrue(UserPreferences.objects.filter(user=self.user1).exists())
    ...
    def test_user_isolation(self):
        ...
        response1 = self._make_request("PATCH", "/api/v1/preferences/me/", self.user1, {"theme": "dark"})
