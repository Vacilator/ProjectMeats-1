...
def outlook_auth_init(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Failed to initialize OAuth flow", "detail": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
class EmailAccountViewSet(viewsets.ModelViewSet):
    ...
    def disconnect(self, request, pk=None):
        ...
        try:
            ...
            return Response({"message": "Account disconnected successfully", "email": account.email_address})
    ...
    def test_connection(self, request, pk=None):
        ...
        try:
            ...
            if account.is_token_expired:
                return Response(
                    {"success": False, "error": "Token expired. Please reconnect."}, status=status.HTTP_401_UNAUTHORIZED
                )

            # TODO: Actually test by sending API request
            return Response({"success": True, "message": f"Connection to {account.email_address} is active"})
