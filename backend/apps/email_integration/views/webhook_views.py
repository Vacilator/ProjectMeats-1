...
def _is_valid_gmail_pubsub_push(request) -> bool:
    ...
    logger.error(
        "Gmail webhook verification not configured; set GMAIL_PUBSUB_VERIFICATION_TOKEN or GOOGLE_PUBSUB_AUDIENCE"
    )
    return False
...
def outlook_webhook_subscribe(request, account_id):
    ...
    try:
        ...
        response = requests.post(graph_url, headers=headers, json=subscription_data, timeout=10)
        ...
        if response.status_code == 201:
            ...
            return Response(
                {"error": "Failed to create subscription", "detail": response.text},
        ...
        return Response(
            {"error": "Internal server error", "detail": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def outlook_webhook_renew(request, account_id):
    ...
    try:
        ...
        if not email_account.webhook_id:
            return Response(
                {"error": "No active subscription", "detail": "Create a subscription first"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        renewal_data = {"expirationDateTime": (timezone.now() + timedelta(days=3)).isoformat()}

        response = requests.patch(graph_url, headers=headers, json=renewal_data, timeout=10)
        ...
        if response.status_code == 200:
            ...
            return Response(
                {"error": "Failed to renew subscription", "detail": response.text},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
def outlook_webhook_unsubscribe(request, account_id):
    ...
    try:
        ...
        if not email_account.webhook_id:
            return Response({"success": True, "message": "No active subscription to delete"})
        ...
        response = requests.delete(graph_url, headers=headers, timeout=10)
        ...
        if response.status_code in [204, 404]:
            ...
            return Response({"success": True, "message": "Webhook subscription deleted successfully"})
        else:
            logger.error(f"Failed to delete Outlook subscription: {response.status_code} - {response.text}")
            return Response(
                {"error": "Failed to delete subscription", "detail": response.text},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
def gmail_webhook_unsubscribe(request, account_id):
    ...
    try:
        ...
        return Response({"success": True, "message": "Gmail watch stopped successfully"})
