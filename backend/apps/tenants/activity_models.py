...
class ActivityLog(models.Model):
    ...
    entity_type = models.CharField(
        max_length=100, null=True, blank=True, help_text='Type of entity affected (e.g., "TenantUser", "Tenant")'
    )
    entity_id = models.CharField(max_length=255, null=True, blank=True, help_text="ID of the affected entity")
    description = models.TextField(help_text="Human-readable description of the action")
    metadata = models.JSONField(
        default=dict, blank=True, help_text="Additional metadata (old values, new values, etc.)"
    )
    ...
    @staticmethod
    def log_activity(
        tenant, user, action, description, entity_type=None, entity_id=None, metadata=None, ip_address=None
    ):
        """
