...
def seed_test_data(environment="development", count=3, verbosity=1):
    ...
    for i in range(1, count + 1):
        ...
        with transaction.atomic():
            ...
            # Create Domain
            TenantDomain.objects.create(tenant=tenant, domain=f"{slug}.localhost", is_primary=True)
            ...
            # Link User to Tenant
            TenantUser.objects.get_or_create(tenant=tenant, user=user, defaults={"role": "admin", "is_active": True})

            # Populate Business Data
            _populate_tenant_business_data(tenant, user, verbosity)

            # Attach credentials for display
            tenant.test_credentials = {"username": admin_username, "password": admin_password, "role": "Admin"}
...
def _populate_tenant_business_data(tenant, user, verbosity=1):
    ...
    try:
        ...
        proteins = ["Bee", "Pork", "Chicken", "Lamb", "Turkey"]
        protein_objs = []
    ...
    try:
        ...
        if not system_products:
            if verbosity >= 1:
                print("  ⚠️  No system products found. Run: python manage.py seed_system_products")
        else:
    ...
    try:
        ...
        for i in range(1, 4):
            ...
            PurchaseOrder.objects.get_or_create(
                tenant=tenant, order_number=f"PO-{tenant.slug}-{i:04d}", defaults=po_defaults  # Globally unique
            )

        if verbosity >= 2:
            print("  📝 Created 3 purchase orders")
