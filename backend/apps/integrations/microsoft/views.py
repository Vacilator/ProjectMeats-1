...
class MicrosoftOAuthCallbackView(APIView):
    ...
    def get(self, request):
            ...
            return Response(
                {
                    ...
                    "detail": "Microsoft OAuth integration is not available. " "Please contact your administrator.",
                    "phase": "Phase 5",
        ...
        if error:
            ...
            return Response(
                {"error": error, "detail": error_description or "Authorization failed"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not auth_code:
            return Response(
                {"error": "missing_code", "detail": "Authorization code not provided in callback"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def post(self, request):
        ...
        if not getattr(settings, "MICROSOFT_CLIENT_ID", None):
            return Response(
                {"error": "Integration not configured", "detail": "Microsoft OAuth not available"},
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )
        ...
        return Response(
            {
                ...
                "features": ["outlook_calendar", "outlook_email", "contact_sync", "event_scheduling"]
                if is_configured
                else [],
            }
        )
