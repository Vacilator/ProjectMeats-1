...
def _build_lineage_metadata(instance: EmailLog, **extra) -> dict:
    ...
    if isinstance(att_data, list) and att_data:
        ...
        metadata["attachment_filenames"] = [str(a.get("filename", "")) for a in att_data if isinstance(a, dict)]
...
def _notify_actionable_email(instance: EmailLog, draft: EmailReviewDraft, classification: dict) -> None:
    ...
    message = f"{subject} from {instance.sender_email} requires review " f'({category.replace("_", " ")}).'
...
def trigger_email_sync_on_login(sender, request, user, **kwargs):
    ...
    """
    tenant_ids = list(TenantUser.objects.filter(user=user, is_active=True).values_list("tenant_id", flat=True))
    if not tenant_ids:
        return
