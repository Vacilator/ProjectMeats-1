...
class ExternalAuthProvider(models.Model):
    ...
    tenant = models.ForeignKey("tenants.Tenant", on_delete=models.CASCADE, related_name="auth_providers")
    provider_type = models.CharField(max_length=20, choices=PROVIDER_CHOICES, help_text="Email provider type")

    # Encrypted token fields
    access_token = models.TextField(help_text="Encrypted OAuth access token")
    refresh_token = models.TextField(blank=True, null=True, help_text="Encrypted OAuth refresh token")
    ...
    connected_email = models.EmailField(blank=True, null=True, help_text="Email address of the connected account")
    connected_name = models.CharField(
        max_length=255, blank=True, null=True, help_text="Display name of the connected account"
    )
...
class EmailLog(models.Model):
    ...
    message_id = models.CharField(
        max_length=255, unique=True, db_index=True, help_text="Unique message ID from provider (prevents duplicates)"
    )
    thread_id = models.CharField(max_length=255, blank=True, null=True, help_text="Thread/conversation ID")
    ...
    extracted_data = models.JSONField(
        blank=True, null=True, help_text="AI-extracted order data (products, quantities, etc.)"
    )
    ...
    related_order_id = models.IntegerField(blank=True, null=True, help_text="ID of order created from this email")

    # Metadata
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)
    updated_at = models.DateTimeField(auto_now=True)
    processed_at = models.DateTimeField(blank=True, null=True, help_text="When AI processing completed")
