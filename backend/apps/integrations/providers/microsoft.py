...
class MicrosoftGraphProvider(EmailProvider):
    ...
    def validate_token(self, access_token: str) -> bool:
        ...
        try:
            ...
            response = requests.get(f"{self.GRAPH_API_BASE}/me", headers=headers, timeout=10)
    ...
    def send_email(self, access_token: str, params: EmailParams) -> EmailSendResult:
        ...
        if params.get("reply_to"):
            message["replyTo"] = [{"emailAddress": {"address": params["reply_to"]}}]
        ...
        if params.get("headers"):
            message["internetMessageHeaders"] = [
                {"name": name, "value": value} for name, value in params["headers"].items()
            ]
        ...
        try:
            draft_response = requests.post(
                f"{self.GRAPH_API_BASE}/me/messages", headers=headers, json=message, timeout=30
            )
