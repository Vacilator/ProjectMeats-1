...
from tenant_apps.inquiries.models import Inquiry, InquiryEntityTypeChoices, InquirySourceChoices, InquiryStatusChoices
...
PROTEIN_LOOKUP = {str(choice.value).strip().lower(): str(choice.value) for choice in ProteinTypeChoices}
...
def upsert_inquiry_draft_from_email(email_log, classification: dict[str, Any]) -> tuple[Inquiry | None, bool]:
    ...
    inquiry = Inquiry.objects.filter(tenant=email_log.tenant, source_email=email_log).order_by("created_on").first()
    ...
    if not inquiry.contact_name:
        inquiry.contact_name = _normalize_text(classification.get("contact_name")) or _normalize_text(
            getattr(email_log, "sender_name", "")
        )
