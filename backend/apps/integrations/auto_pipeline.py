...
def _log_pipeline_event(tenant_id: str, email_log_id: int, step: str, status: str, payload: dict | None = None):
    ...
    logger.info(
        ...
        step, tenant_id, email_log_id, status, payload or {},
    )
        ...
        for tenant_id in tenant_ids:
            ...
            for email_log_id, extracted_data in candidates:
                ...
                if confidence < AUTO_PROCESS_CONFIDENCE_THRESHOLD:
                    _log_pipeline_event(
                        tid, email_log_id, 'skip_low_confidence', 'info',
                ...
                if draft_type != 'purchase_order':
                    _log_pipeline_event(
                        tid, email_log_id, 'skip_non_po', 'info',
                ...
                _log_pipeline_event(
                    tid, email_log_id, 'pipeline_dispatched', 'started',
    ...
    try:
        with tenant_rls(tenant_id):
            ...
            if email_log.related_order_id and email_log.status == 'order_created':
                _log_pipeline_event(
                    tenant_id, email_log_id, 'create_po', 'skipped',
            ...
            if contact_company:
                ...
                if not supplier:
                    ...
                    _log_pipeline_event(
                        tenant_id, email_log_id, 'auto_create_supplier', 'success',
            ...
            _log_pipeline_event(
                tenant_id, email_log_id, 'create_po', 'success',
    ...
    try:
        with tenant_rls(tenant_id):
            ...
            if existing_inquiry and existing_inquiry.sales_order:
                _log_pipeline_event(
                    tenant_id, po_id, 'generate_so', 'skipped',
            ...
            _log_pipeline_event(
                tenant_id, po_id, 'generate_so', 'success',
    ...
    try:
        with tenant_rls(tenant_id):
            ...
            if so.status in TERMINAL_STATUSES:
                _log_pipeline_event(
                    tenant_id, so_id, 'trigger_fulfillment', 'skipped',
            ...
            _log_pipeline_event(
                tenant_id, so_id, 'trigger_fulfillment', 'success',
