...
class Fulfillment(SoftDeleteModel, TenantAwareModel):
    ...
    fulfillment_number = models.CharField(
        max_length=20, editable=False, help_text="Auto-generated fulfillment number (FUL-YYYY-NNNNN)"
    )
    ...
    # Logistics and dates
    ship_date = models.DateField(null=True, blank=True, help_text="When the shipment was sent")
    ...
    tracking_numbers = ArrayField(
        models.CharField(max_length=100), blank=True, default=list, help_text="Tracking numbers for this shipment"
    )
    ...
    # Notes
    notes = models.TextField(blank=True, default="", help_text="Fulfillment notes")
    ...
    def _generate_fulfillment_number(self):
        ...
        last_fulfillment = (
            Fulfillment.objects.filter(tenant=self.tenant, fulfillment_number__startswith=prefix)
    ...
    def total_value(self):
        ...
        return self.products.aggregate(total=models.Sum("total"))["total"] or Decimal("0.00")
...
class FulfillmentProduct(TenantAwareModel):
    ...
    fulfillment = models.ForeignKey(Fulfillment, on_delete=models.CASCADE, related_name="products")
    ...
    quantity_fulfilled = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal("0.00"), help_text="Quantity being fulfilled in this shipment"
    )

    # Pricing for this fulfillment
    unit_price = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, help_text="Price per unit for this fulfillment"
    )
    total = models.DecimalField(
        max_digits=12, decimal_places=2, null=True, blank=True, help_text="Line total for this fulfillment"
    )
    ...
    def __str__(self):
        ...
        product_name = (
            getattr(product, "name", "").strip() or getattr(product, "product_code", "").strip() or "Product"
        )[:30]
