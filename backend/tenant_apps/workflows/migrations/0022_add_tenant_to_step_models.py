...
def backfill_tenant_for_formstepsubmission(apps, schema_editor):
    ...
    print(
        f"✓ Backfilled tenant for {FormStepSubmission.objects.filter(tenant__isnull=False).count()} FormStepSubmission records"
    )
...
def backfill_tenant_for_stepassignment(apps, schema_editor):
    ...
    print(
        f"✓ Backfilled tenant for {StepAssignment.objects.filter(tenant__isnull=False).count()} StepAssignment records"
    )
