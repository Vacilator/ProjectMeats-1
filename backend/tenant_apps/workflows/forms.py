...
class IconSelectorWidget(forms.TextInput):
    ...
    class Media:
        css = {"all": ("admin/workflows/css/icon_selector.css",)}
...
class TenantFormEntityForm(forms.ModelForm):
    ...
    entity_type = forms.ChoiceField(
        choices=[("", "---------")] + ENTITY_TYPES, help_text="Select the entity type for this form step"
    )
    ...
    class Meta:
        ...
        widgets = {
            "step_name": forms.TextInput(attrs={"placeholder": "e.g., Customer Information, Order Details"}),
        }
        help_texts = {
...
class TenantFormFieldForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "custom_label": forms.TextInput(attrs={"placeholder": "Custom label (optional)"}),
            "custom_help_text": forms.TextInput(attrs={"placeholder": "Help text shown below field"}),
        }
...
class TenantFormRuleForm(forms.ModelForm):
    ...
    condition_operator = forms.ChoiceField(
        choices=OperatorType.choices, required=False, initial=OperatorType.EQUALS, label="Has this condition"
    )
    ...
    action_type = forms.ChoiceField(
        choices=[("", "-- Select Action --")] + list(ActionType.choices), required=False, label="Then perform action"
    )
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(attrs={"placeholder": "e.g., Show shipping fields when delivery selected"}),
        }
        help_texts = {
...
class TenantWorkflowAdminForm(forms.ModelForm):
    ...
    schedule_description = forms.CharField(
        ...
        required=False,
        label="Schedule",
        widget=forms.TextInput(attrs={"placeholder": "e.g., Every Monday at 9am, Daily at midnight"}),
...
class TenantWorkflowConditionForm(forms.ModelForm):
    ...
    field_path = forms.ChoiceField(
        choices=[("", "-- Select Field --")] + SYSTEM_FIELDS + [("custom", "↳ Custom field path...")],
...
class TenantWorkflowActionForm(forms.ModelForm):
    ...
    email_body = forms.CharField(
        ...
        label="Body",
        widget=forms.Textarea(
            attrs={"rows": 4, "placeholder": "Hello {{record.customer.name}},\n\nYour order has been updated."}
        ),
    )
    ...
    notification_message = forms.CharField(max_length=500, required=False, label="Message")

    # Set value fields
    set_field = forms.CharField(
        max_length=100, required=False, label="Field to set", widget=forms.TextInput(attrs={"placeholder": "status"})
    )

    set_value = forms.CharField(
        max_length=255, required=False, label="Value to set", widget=forms.TextInput(attrs={"placeholder": "approved"})
    )
...
class QuickAddFormRuleForm(forms.Form):
    ...
    name = forms.CharField(
        max_length=255, required=False, widget=forms.TextInput(attrs={"placeholder": "Rule name (optional)"})
    )

    when_field = forms.ChoiceField(choices=[("", "-- Select --")] + SYSTEM_FIELDS, label="When")

    operator = forms.ChoiceField(choices=OperatorType.choices, initial=OperatorType.EQUALS)

    value = forms.CharField(max_length=255, widget=forms.TextInput(attrs={"placeholder": "Value"}))

    then_action = forms.ChoiceField(choices=ActionType.choices, label="Then")
...
class QuickAddWorkflowActionForm(forms.Form):
    ...
    action_type = forms.ChoiceField(choices=ActionType.choices, label="Action")
    ...
    continue_on_error = forms.BooleanField(required=False, label="Continue if this action fails")
