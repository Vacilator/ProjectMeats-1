        ...
        try:
            ...
            try:
                auth_provider = ExternalAuthProvider.objects.get(
                    tenant_id=tenant_id, provider_type="microsoft", is_active=True
                )
            ...
            to_addresses = [self.render_template(addr, context) for addr in self.config["to"]]
            ...
            if "cc" in self.config:
                cc_addresses = [self.render_template(addr, context) for addr in self.config["cc"]]
                email_params["cc"] = self.validate_email_addresses(cc_addresses)

            if "bcc" in self.config:
                bcc_addresses = [self.render_template(addr, context) for addr in self.config["bcc"]]
                email_params["bcc"] = self.validate_email_addresses(bcc_addresses)

            if "body_html" in self.config:
                email_params["body_html"] = self.render_template(self.config["body_html"], context)
            ...
            logger.info(
                f"Email sent successfully via node {self.node_id}: " f"to={email_params['to']}, subject={subject}"
            )
