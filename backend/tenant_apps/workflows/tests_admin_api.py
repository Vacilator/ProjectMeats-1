...
from tenant_apps.workflows.services import FieldRegistry, get_available_entities, get_entity_fields
...
class AdminAPITests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.form = TenantForm.objects.create(
            tenant=cls.tenant, name="Test Form", description="A test form", status="draft"
        )
    ...
    def test_save_step_fields_api(self):
        ...
        response = self.client.post(f"/api/v1/workflows/admin/steps/{self.step.id}/fields/", fields_data, format="json")
    ...
    def test_reorder_steps_api(self):
        ...
        step2 = TenantFormEntity.objects.create(
            form=self.form, entity_type="contact", step_name="Contact Info", order=1
        )
    ...
    def test_smart_match_api(self):
        ...
        response = self.client.post(
            ...
            {"source_field": {"key": "email", "type": "email"}, "target_entity_type": "contact"},
            format="json",
        )
    ...
    def test_save_empty_fields_list(self):
        ...
        TenantFormField.objects.create(form_entity=self.step, field_key="name", order=0)
        TenantFormField.objects.create(form_entity=self.step, field_key="email", order=1)
        self.assertEqual(self.step.fields.count(), 2)

        # Now save empty list
        response = self.client.post(
            f"/api/v1/workflows/admin/steps/{self.step.id}/fields/", {"fields": []}, format="json"
        )
    ...
    def test_save_fields_replaces_existing(self):
        ...
        # Add initial field
        TenantFormField.objects.create(form_entity=self.step, field_key="old_field", order=0)
    ...
    def test_get_all_entity_types(self):
        ...
        for entity in entities:
            ...
            self.assertEqual(response.status_code, status.HTTP_200_OK, f'Failed for entity: {entity["key"]}')
            self.assertIn("fields", response.data)
    ...
    def test_smart_match_with_various_field_types(self):
        ...
        for source_field in test_cases:
            response = self.client.post(
                "/api/v1/workflows/admin/smart-match/",
                {"source_field": source_field, "target_entity_type": "customer"},
                format="json",
            )

            self.assertEqual(response.status_code, status.HTTP_200_OK, f'Failed for field type: {source_field["type"]}')
...
class FieldConfigAPITests(APITestCase):
    ...
    def setUpTestData(cls):
        # Create admin user
        cls.admin_user = User.objects.create_superuser(
            username="configadmin", email="configadmin@test.com", password="testpass123"
        )
        ...
        # Create test form
        cls.form = TenantForm.objects.create(
            tenant=cls.tenant, name="Config Test Form", description="A test form for config tests", status="draft"
        )
        ...
        cls.step2 = TenantFormEntity.objects.create(
            form=cls.form, entity_type="contact", step_name="Contact Info", order=1
        )

        # Create a field in step 1
        cls.source_field = TenantFormField.objects.create(form_entity=cls.step1, field_key="email", order=0)

        # Create a field in step 2 (target for config)
        cls.target_field = TenantFormField.objects.create(form_entity=cls.step2, field_key="email", order=0)
    ...
    def test_save_field_config(self):
        ...
        response = self.client.post(
            f"/api/v1/workflows/admin/fields/{self.target_field.id}/config/",
            {
                ...
                "auto_populate": {"source_step": str(self.step1.id), "source_field": "email", "mode": "copy"},
            },
            format="json",
        )
    ...
    def test_clear_auto_populate(self):
        ...
        response = self.client.post(
            f"/api/v1/workflows/admin/fields/{self.target_field.id}/config/",
            {"auto_populate": {"source_step": None, "source_field": "", "mode": ""}},
            format="json",
        )
...
class FormRulesAPITests(APITestCase):
    ...
    def setUpTestData(cls):
        # Create admin user
        cls.admin_user = User.objects.create_superuser(
            username="rulesadmin", email="rulesadmin@test.com", password="testpass123"
        )
        ...
        # Create test form
        cls.form = TenantForm.objects.create(
            tenant=cls.tenant, name="Rules Test Form", description="A test form for rules tests", status="draft"
        )
        ...
        # Create step 2
        cls.step2 = TenantFormEntity.objects.create(
            form=cls.form, entity_type="customer", step_name="Customer Info", order=1
        )
    ...
    def test_create_rule(self):
        ...
        response = self.client.post(
            ...
            {
                "name": "Test Rule",
                "conditions": [{"field": f"{self.step1.id}.supplier_type", "operator": "eq", "value": "wholesale"}],
                "condition_logic": "and",
                "actions": [{"action": "display_fields", "params": {"fields": [f"{self.step2.id}.credit_limit"]}}],
            },
            format="json",
        )
    ...
    def test_create_rule_auto_order(self):
        ...
        # Create first rule
        rule1 = TenantFormRule.objects.create(form=self.form, name="Rule 1", conditions=[], actions=[], order=0)
        ...
        rule2 = TenantFormRule.objects.create(
            form=self.form, name="Rule 2", conditions=[], actions=[], order=next_order
        )
    ...
    def test_update_rule(self):
        ...
        # Create a rule
        rule = TenantFormRule.objects.create(form=self.form, name="Original Name", conditions=[], actions=[], order=0)
    ...
    def test_delete_rule(self):
        ...
        rule = TenantFormRule.objects.create(form=self.form, name="To Delete", conditions=[], actions=[], order=0)
        rule_id = rule.id
    ...
    def test_update_nonexistent_rule(self):
        ...
        response = self.client.put(f"/api/v1/workflows/admin/rules/{uuid.uuid4()}/", {"name": "Test"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class FormStepsAPITests(APITestCase):
    ...
    def setUpTestData(cls):
        # Create admin user
        cls.admin_user = User.objects.create_superuser(
            username="stepsadmin", email="stepsadmin@test.com", password="testpass123"
        )
        ...
        # Create test form
        cls.form = TenantForm.objects.create(
            tenant=cls.tenant, name="Steps Test Form", description="A test form for steps tests", status="draft"
        )
    ...
    def test_get_form_steps(self):
        ...
        TenantFormEntity.objects.create(form=self.form, entity_type="supplier", step_name="Step 1", order=0)
        TenantFormEntity.objects.create(form=self.form, entity_type="customer", step_name="Step 2", order=1)
    ...
    def test_create_step(self):
        ...
        response = self.client.post(
            ...
            {"entity_type": "product", "step_name": "Product Details"},
            format="json",
        )
    ...
    def test_create_step_auto_order(self):
        ...
        response1 = self.client.post(
            f"/api/v1/workflows/admin/forms/{self.form.id}/steps/", {"entity_type": "supplier"}, format="json"
        )
        self.assertEqual(response1.data["step"]["order"], 0)

        # Create second step
        response2 = self.client.post(
            f"/api/v1/workflows/admin/forms/{self.form.id}/steps/", {"entity_type": "customer"}, format="json"
        )
        self.assertEqual(response2.data["step"]["order"], 1)

        # Create third step
        response3 = self.client.post(
            f"/api/v1/workflows/admin/forms/{self.form.id}/steps/", {"entity_type": "product"}, format="json"
        )
    ...
    def test_create_step_missing_entity_type(self):
        ...
        response = self.client.post(
            f"/api/v1/workflows/admin/forms/{self.form.id}/steps/", {"step_name": "No Entity"}, format="json"
        )
    ...
    def test_delete_step(self):
        ...
        step = TenantFormEntity.objects.create(form=self.form, entity_type="carrier", step_name="To Delete", order=0)
