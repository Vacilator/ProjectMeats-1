...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not tenant_id and not all_tenants:
            self.stdout.write(self.style.ERROR("Error: Must specify --tenant=<uuid> or --all-tenants"))
            return 1
        ...
        total_stats = {"processed": 0, "orders_triggered": 0, "inquiries": 0, "errors": 0}
        ...
        for tenant in tenants:
            ...
            if stats["orders_triggered"] > 0:
                self.stdout.write(self.style.SUCCESS(f'  ✓ Orders Triggered: {stats["orders_triggered"]}'))
        ...
        if total_stats["errors"] > 0:
            self.stdout.write(self.style.WARNING(f'  Errors: {total_stats["errors"]}'))
