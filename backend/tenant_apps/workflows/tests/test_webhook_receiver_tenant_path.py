...
class WorkflowWebhookTenantPathTests(TestCase):
    ...
    def test_tenant_scoped_webhook_executes_and_sets_rls(self, execute_workflow, set_current_tenant):
        ...
        url = f"/api/v1/tenants/{self.tenant.id}/workflows/webhooks/" f"{self.workflow.id}/{self.webhook_token}/"
        ...
        with patch(
            "tenant_apps.workflows.views_triggers.TenantWorkflow.objects.select_related", side_effect=select_related_spy
        ):
    ...
    def test_tenant_mismatch_fails_closed(self, execute_workflow, set_current_tenant):
        ...
        url = f"/api/v1/tenants/{self.other_tenant.id}/workflows/webhooks/" f"{self.workflow.id}/{self.webhook_token}/"
    ...
    def test_invalid_token_fails_closed(self, execute_workflow, set_current_tenant):
        ...
        url = f"/api/v1/tenants/{self.tenant.id}/workflows/webhooks/" f"{self.workflow.id}/wrong-token/"
    ...
    def test_public_webhooks_ignore_global_throttles(self, execute_workflow, set_current_tenant):
        ...
        tenant_url = f"/api/v1/tenants/{self.tenant.id}/workflows/webhooks/" f"{self.workflow.id}/{self.webhook_token}/"
