    ...
    with transaction.atomic():
        ...
        if step_submissions:
            ...
            if first_step and not instance.current_step:
                ...
                FormStepSubmission.objects.filter(submission=instance, step=first_step).update(
                    status=StepSubmissionStatus.IN_PROGRESS
                )
...
def _cache_previous_step_submission_status(sender, instance: FormStepSubmission, **kwargs):
    ...
    try:
        instance._previous_status = FormStepSubmission.objects.only("status").get(pk=instance.pk).status
...
def notify_action_needed_assignee(sender, instance: FormStepSubmission, created: bool, **kwargs):
    ...
    try:
        ...
        step_label = (
            getattr(instance.step, "step_name", None) or getattr(instance.step, "name", None) or str(instance.step)
        )
...
def _build_form_snapshot(form):
    ...
    for entity in form.entities.all().order_by("order"):
        ...
        for field in entity.fields.filter(is_visible=True).order_by("order"):
            ...
            field_type = (
                field.field_type if field.field_type and field.field_type != "text" else field_meta.get("type", "text")
            )
...
def trigger_event_workflows(sender, instance, created=False, **kwargs):
    ...
    entity_type = _get_entity_type_from_model(
        sender._meta.app_label.replace("tenant_apps.", ""), sender._meta.model_name
    )
    ...
    try:
        ...
        for workflow in workflows:
            ...
            if event_filter in [event_type, "any"]:
                logger.info(
                    f"[Event] Triggering workflow {workflow.name} " f"for {entity_type}:{instance.id} ({event_type})"
                )
...
def register_entity_signals():
    ...
    for model in entity_models:
        post_save.connect(trigger_event_workflows, sender=model, dispatch_uid=f"workflow_trigger_{model._meta.label}")
