...
from .forms import (
    TenantFormAdminForm,
    TenantFormEntityForm,
    TenantFormFieldForm,
    TenantFormRuleForm,
    TenantListForm,
    TenantWorkflowActionForm,
    TenantWorkflowAdminForm,
    TenantWorkflowConditionForm,
)
from .models import (
    FormStatus,
    TenantForm,
...
class TenantFormAdmin(TenantFilteredAdmin):
    ...
    list_display = ["name", "tenant", "status_badge", "entity_count", "is_default", "quick_action_badge", "created_at"]
    ...
    def export_forms(self, request, queryset):
        ...
        if queryset.count() == 1:
            ...
            response = HttpResponse(
                json.dumps(export_data, indent=2, ensure_ascii=False), content_type="application/json"
            )
            ...
            response = HttpResponse(
                json.dumps({"forms": exports}, indent=2, ensure_ascii=False), content_type="application/json"
            )
...
class TenantFormEntityAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        return (
            super().get_queryset(request).annotate(_field_count=Count("fields")).select_related("form", "form__tenant")
        )
...
class TenantWorkflowAdmin(TenantFilteredAdmin):
    ...
    list_display = ["name", "tenant", "status_badge", "trigger_badge", "entity_type", "run_count", "last_run_at"]
    ...
    fieldsets = [
        ...
        ("Execution Stats", {"fields": ("run_count", "last_run_at"), "classes": ["collapse"]}),
        ("Metadata", {"fields": ("id", "created_by", "created_at", "updated_at"), "classes": ["collapse"]}),
    ]
    ...
    def run_workflow_manually(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Manual execution queued for {count} workflow(s). " "Note: Workflow engine integration pending.",
            messages.INFO,
        )
...
class FormSubmissionAdmin(TenantFilteredAdmin):
    ...
    def progress_display(self, obj):
        ...
        return format_html('<span title="{}% complete">{}/{} steps</span>', percent, completed, total)
...
class FormStepSubmissionAdmin(TenantFilteredAdmin):
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related("submission", "submission__form", "step", "completed_by")
