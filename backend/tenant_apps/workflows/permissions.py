...
class WorkFormPermissionHelper:
    ...
    def get_permissions_for_user(user, tenant) -> dict:
        ...
        logger.info(
            f"[WorkFormPermissions] Checking permissions for user: {user}, is_authenticated: {user.is_authenticated if user else False}, is_superuser: {user.is_superuser if user else False}, tenant: {tenant}"
        )
        ...
        try:
            ...
            logger.warning(
                f"[WorkFormPermissions] TenantUser not found for user {user.id} in tenant {tenant.id if tenant else None}"
            )
            return WorkFormPermissionHelper._get_anonymous_permissions()
