...
class WorkflowExecutor:
    ...
    def _evaluate_single_condition(self, condition: TenantWorkflowCondition) -> bool:
        ...
        if (compare_value is None or not isinstance(compare_value, (str, int, float, bool, list, dict))) and hasattr(
            condition, "value"
        ):
    ...
    def _execute_update_record(self, action: TenantWorkflowAction) -> None:
        config = action.config or {}
        logger.info(
            "Update record: model=%s id=%s updates=%s",
            config.get("model"),
    ...
    def _record_action_failure(
        self, action: TenantWorkflowAction, error: Exception, extra: Optional[Dict[str, Any]] = None
    ) -> None:
        if not self.execution_log:
            return
        self._ensure_log_defaults()
