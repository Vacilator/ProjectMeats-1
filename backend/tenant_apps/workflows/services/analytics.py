...
from ..models import FormSubmission, FormSubmissionEvent, FormSubmissionEventType, FormSubmissionStatus, TenantForm


def get_form_analytics(form: TenantForm, days: int = 30, include_events: bool = True) -> Dict[str, Any]:
    """
    ...
    # Base queryset
    submissions = FormSubmission.objects.filter(form=form, created_at__gte=cutoff)

    # Count by status
    status_counts = dict(submissions.values("status").annotate(count=Count("id")).values_list("status", "count"))
    ...
    avg_completion = (
        submissions.filter(status=FormSubmissionStatus.COMPLETED, completed_at__isnull=False)
    ...
    submissions_by_day = list(
        submissions.annotate(date=TruncDate("created_at"))
        .values("date")
        .annotate(count=Count("id"), completed=Count("id", filter=Q(status=FormSubmissionStatus.COMPLETED)))
        .order_by("date")
    )
    ...
    if include_events:
        events = FormSubmissionEvent.objects.filter(submission__form=form, created_at__gte=cutoff)
        ...
        field_errors = list(
            events.filter(event_type=FormSubmissionEventType.FIELD_ERROR)
            .values("field_key")
        ...
        analytics["field_errors"] = [
            {"field": item["field_key"], "errors": item["error_count"]} for item in field_errors
        ]

        # Validation error types
        validation_errors = events.filter(event_type=FormSubmissionEventType.VALIDATION_ERROR).count()
...
def _get_step_metrics(form: TenantForm, events, submissions) -> List[Dict[str, Any]]:
    ...
    for step in steps:
        ...
        entered = events.filter(event_type=FormSubmissionEventType.STEP_ENTERED, step_id=step.id).count()

        # Count how many times this step was completed
        completed = events.filter(event_type=FormSubmissionEventType.STEP_COMPLETED, step_id=step.id).count()

        # Average time spent on this step
        step_times = events.filter(
            event_type=FormSubmissionEventType.STEP_COMPLETED, step_id=step.id, duration_ms__isnull=False
...
def get_tenant_form_summary(tenant, days: int = 30) -> Dict[str, Any]:
    ...
    return {
        ...
        "overall_completion_rate": round(total_completed / total_submissions * 100 if total_submissions > 0 else 0, 1),
