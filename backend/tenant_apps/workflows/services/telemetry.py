...
def _normalize_runtime_event_rows(execution: TenantWorkFormExecution, *, rows: Iterable[Any] | None = None) -> list[dict[str, Any]]:
    normalized: list[dict[str, Any]] = []

    if rows is None:
        persisted_rows = list(
            ExecutionEventLog.objects.filter(workform_execution=execution).order_by('sequence')
        )
        if persisted_rows:
            rows = persisted_rows
        else:
    ...
    for index, row in enumerate(rows):
        ...
        payload = {
            ...
            if key not in {'ts', 'event', 'event_type', 'node_id', 'node_type'}
        }
        normalized.append(
            {
                'sequence': index,
