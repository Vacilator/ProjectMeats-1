...
class FormVersionService:
    ...
    @staticmethod
    def create_version(form: TenantForm, change_summary: str, user: Optional[User] = None) -> TenantFormVersion:
        """
    ...
    @staticmethod
    def rollback_to_version(form: TenantForm, version_number: int, user: Optional[User] = None) -> TenantFormVersion:
        """
        ...
        with transaction.atomic():
            ...
            rollback_version = FormVersionService.create_version(
                form=form, change_summary=f"Rollback to version {version_number}", user=user
            )
    ...
    @staticmethod
    def compare_versions(form: TenantForm, version_a: int, version_b: int) -> Dict[str, Any]:
        """
        ...
        if flow_a != flow_b:
            ...
            diff["flow_data"] = {"added_nodes": added_nodes, "removed_nodes": removed_nodes}
    ...
    def enable_versioning(form: TenantForm, user: Optional[User] = None) -> TenantFormVersion:
        ...
        return FormVersionService.create_version(
            form=form, change_summary="Initial version (versioning enabled)", user=user
        )
