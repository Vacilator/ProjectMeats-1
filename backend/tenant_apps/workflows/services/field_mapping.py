...
def calculate_mapping_score(
    target_field: Dict[str, Any], source_field: Dict[str, Any], step_distance: int
    ...
    name_sim = calculate_name_similarity(target_field.get("key", ""), source_field.get("key", ""))
...
def find_best_mapping(
    target_field: Dict[str, Any], source_fields: List[Dict[str, Any]], min_score: float = 50.0
    ...
    for source in source_fields:
        ...
        if score > best_score:
            best_score = score
            best_match = {**source, "score": score, "reasons": reasons}
...
class FieldMappingService:
    ...
    def get_source_fields_for_step(form, target_step_order: int) -> List[Dict[str, Any]]:
        ...
        prior_steps = form.entities.filter(order__lt=target_step_order).order_by("-order")  # Most recent first

        for step in prior_steps:
            step_distance = target_step_order - step.order

            # Get fields configured for this step
            step_fields = TenantFormField.objects.filter(form_entity=step, is_visible=True).select_related(
                "form_entity"
            )
    ...
    def compute_auto_mappings(form, min_score: float = 50.0) -> List[Dict[str, Any]]:
        ...
        for step in steps:
            ...
            step_fields = TenantFormField.objects.filter(form_entity=step, is_visible=True).filter(
                Q(auto_populate_source_step__isnull=True) | Q(auto_populate_source_field="")
            )
    ...
    def get_current_mappings(form) -> List[Dict[str, Any]]:
        ...
        fields = (
            TenantFormField.objects.filter(form_entity__form=form, auto_populate_source_step__isnull=False)
            .exclude(auto_populate_source_field="")
    ...
    def apply_mapping(field, source_step_id: str, source_field_key: str, mode: str = "copy") -> bool:
        ...
        try:
            ...
            field.save(update_fields=["auto_populate_source_step", "auto_populate_source_field", "auto_populate_mode"])
            return True
        ...
        for mapping in mappings:
            try:
                ...
                success = FieldMappingService.apply_mapping(
                    field, mapping["source_step_id"], mapping["source_field_key"], mapping.get("mode", "copy")
                )
                if success:
                    applied += 1
                else:
