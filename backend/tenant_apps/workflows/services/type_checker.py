...
class TypeChecker:
    ...
    def validate_field(self, field_name: str, value: Any) -> Any:
        ...
        if field_name not in self.field_definitions:
            raise ValidationError(field_name, "unknown", value, f"Field '{field_name}' not found in schema")
        ...
        if required and (value is None or value == ""):
            raise ValidationError(field_name, field_type, value, f"Field '{field_name}' is required but was empty")
        ...
        if field_type in ("integer", "number", "decimal"):
            ...
            if min_val is not None and value < min_val:
                raise ValidationError(field_name, field_type, value, f"Value {value} is less than minimum {min_val}")
            if max_val is not None and value > max_val:
                raise ValidationError(field_name, field_type, value, f"Value {value} is greater than maximum {max_val}")
    ...
    def _validate_email(self, field_name: str, value: Any) -> str:
        ...
        if not self.EMAIL_PATTERN.match(value):
            raise ValidationError(field_name, "email", value, f"Invalid email format: {value}")
    ...
    def _validate_phone(self, field_name: str, value: Any) -> str:
        ...
        if not self.PHONE_PATTERN.match(cleaned):
            raise ValidationError(
                field_name, "phone", value, f"Invalid phone number format: {value} (expected E.164 format)"
            )
    ...
    def _validate_url(self, field_name: str, value: Any) -> str:
        ...
        if not self.URL_PATTERN.match(value):
            raise ValidationError(field_name, "url", value, f"Invalid URL format: {value}")
    ...
    def _validate_choice(self, field_name: str, value: Any, choices: List[Any]) -> Any:
        ...
        if not choices:
            raise ValidationError(field_name, "choice", value, f"No choices defined for field '{field_name}'")

        if value not in choices:
            raise ValidationError(
                field_name, "choice", value, f"Value '{value}' is not a valid choice. Must be one of: {choices}"
            )
