...
class WorkflowGraphValidator:
    ...
    def _validate_empty_workflow(self):
        ...
        if not self.nodes:
            self.errors.append(
                ValidationError(
                    node_id="", error_type="empty_workflow", message="Workflow has no nodes", severity="error"
                )
            )
            ...
            for node_id in start_nodes[1:]:
                self.errors.append(
                    ValidationError(
                        ...
                        message="Workflow has multiple Start nodes. Only one Start node is recommended.",
                        severity="warning",
                    )
                )
    ...
    def _validate_invalid_connections(self):
        ...
        for node_id in self.node_map.keys():
            ...
            if node_type in {"conditional", "conditionalbranch", "i", "branch"}:
                if outgoing_count != 2:
                    node_label = self.node_map[node_id].get("data", {}).get("label", node_id)
                    self.errors.append(
                        ValidationError(
