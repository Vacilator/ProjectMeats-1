        ...
        for field in fields:
            ...
            if field.auto_populate_mode:
                field_data["auto_populate"] = {
                    ...
                    "source_step_order": field.auto_populate_source_step.order
...
def validate_import_data(data: Dict[str, Any]) -> List[str]:
    ...
    if form_data.get("entity_type") not in valid_entity_types:
        errors.append(
            f"Invalid entity_type: {form_data.get('entity_type')}. Valid types: {', '.join(valid_entity_types)}"
        )
...
def import_form(data: Dict[str, Any], tenant, name_suffix: str = " (Imported)", created_by=None) -> TenantForm:
    """
...
def import_form_json(json_string: str, tenant, name_suffix: str = " (Imported)", created_by=None) -> TenantForm:
    """
...
def duplicate_form(form: TenantForm, target_tenant=None, new_name: Optional[str] = None) -> TenantForm:
    """
