...
class FormProcessPersistenceService:
    ...
    def sync_form_process_group(
        ...
        try:
            ...
            container_name = (
                group_data.get('containerName') or 
                group_data.get('label') or 
                group_data.get('name') or
                'Unnamed Form Process'
            )
            ...
            for step in child_steps:
                ...
                entity_type = (
                    step_data.get('entityType') or 
                    step_data.get('entity_type') or
                    'unknown'
                )
                ...
                form_structure['steps'].append({
                    ...
                    'step_name': step_data.get('label', entity_type),
                    'fields': fields
                })
            ...
            try:
                ...
                if form_id:
                    existing_form = TenantForm.objects.get(
                        id=form_id,
                        tenant=self.tenant
                    )
