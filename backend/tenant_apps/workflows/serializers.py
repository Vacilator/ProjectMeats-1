...
class TenantListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TenantList
        fields = ["id", "name", "description", "options", "option_count", "is_active", "created_at", "updated_at"]
...
class TenantFormFieldSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "auto_populate_mode",
            "auto_populate_source_step_name",
            "cascade_parent_field",
            "cascade_filter_key",
            "cascade_enabled",
...
class TenantFormEntitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "entity_type", "step_name", "order", "fields", "field_count"]
        read_only_fields = ["id"]
...
class TenantFormRuleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "name", "is_active", "order", "conditions", "condition_logic", "actions"]
        read_only_fields = ["id"]
...
class TenantFormSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "can_edit",
            "can_publish",
            "can_delete",  # Phase 4.2: Permission metadata
            "created_at",
            "updated_at",
            "flow_data",  # Phase 2.1: Visual editor
        ]
...
class TenantFormCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TenantForm
        fields = ["name", "description", "status", "is_default", "icon", "entities", "flow_data"]
...
class TenantWorkflowConditionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "field_path", "operator", "operator_display", "compare_value", "order"]
        read_only_fields = ["id"]
...
class TenantWorkflowActionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "action_type", "action_type_display", "config", "order", "continue_on_error"]
        read_only_fields = ["id"]
...
from .models import FormStepSubmission, FormSubmission
...
class FormSubmissionListSerializer(serializers.ModelSerializer):
    ...
    def get_progress(self, obj):
        ...
        return {"completed": completed, "total": total, "percent": obj.progress_percent}
...
class AvailableFormSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TenantForm
        fields = ["id", "name", "description", "icon", "status", "is_default", "is_quick_action_enabled", "step_count"]
        # Note: Used with ReadOnlyModelViewSet, so all fields are read-only by design
        read_only_fields = ["id", "name", "description", "icon", "status", "is_default", "is_quick_action_enabled"]
...
from .models import FormStatusHistory, StepAssignment, UserNotification, UserNotificationPreferences
...
class UserNotificationSerializer(serializers.ModelSerializer):
    ...
    def get_action_url(self, obj):
        ...
        feedback_id = str(
            metadata.get("feedback_id") or metadata.get("draft_id") or metadata.get("review_id") or ""
        ).strip()
        ...
        looks_like_ai_review = action_url.startswith("/settings/email-integrations") and (
            any(
