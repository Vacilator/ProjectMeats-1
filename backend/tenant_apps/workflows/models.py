...
class TenantList(TenantAwareModel):
    ...
    name = models.CharField(max_length=255, help_text="Name of the option list")
    description = models.TextField(blank=True, default="", help_text="Description of the list")

    # Options stored as JSON array of {value, label} objects
    options = models.JSONField(default=list, help_text="List of options [{value: 'v1', label: 'Label 1'}, ...]")
...
class TenantForm(TenantAwareModel):
    ...
    description = models.TextField(blank=True, default="", help_text="Description of the form's purpose")

    # Status
    status = models.CharField(
        max_length=20, choices=FormStatus.choices, default=FormStatus.DRAFT, help_text="Current status of the form"
    )

    # If true, this form is used as the default when creating records
    # Only applicable for single-entity forms
    is_default = models.BooleanField(
        default=False, help_text="Use as default form for entity creation (single-entity forms only)"
    )

    # Icon and styling
    icon = models.CharField(max_length=50, blank=True, default="file-text", help_text="Icon identifier for UI")

    # Quick Actions availability
    is_quick_action_enabled = models.BooleanField(
        default=False, help_text="Allow this form to be added to user Quick Actions menu"
    )

    # Template Library (Phase 2.2)
    is_system_template = models.BooleanField(
        default=False, help_text="True if this is an industry-standard system template"
    )
    ...
    flow_data = models.JSONField(default=dict, blank=True, help_text="Visual editor flow definition (nodes and edges)")

    # Version control fields (Phase 2.4)
    version_enabled = models.BooleanField(default=False, help_text="Enable version control for this form")
...
class TenantFormVersion(TenantAwareModel):
    ...
    form = models.ForeignKey(
        TenantForm, on_delete=models.CASCADE, related_name="versions", help_text="Form this version belongs to"
    )

    version_number = models.PositiveIntegerField(help_text="Sequential version number (1, 2, 3...)")

    snapshot_data = models.JSONField(help_text="Complete snapshot of form configuration at this version")

    change_summary = models.TextField(blank=True, default="", help_text="Summary of changes made in this version")
    ...
    is_current = models.BooleanField(default=False, help_text="Whether this is the current active version")
...
class TenantFormEntity(TenantAwareModel):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    form = models.ForeignKey(
        TenantForm, on_delete=models.CASCADE, related_name="entities", help_text="Form this entity belongs to"
    )
    ...
    entity_type = models.CharField(max_length=100, help_text="Type of entity (model name in snake_case)")

    # Display name for this step
    step_name = models.CharField(
        max_length=255, blank=True, default="", help_text="Custom name for this step (defaults to entity name)"
    )

    # Order in the form (for multi-entity progressive forms)
    order = models.PositiveIntegerField(default=0, help_text="Order of this entity in the form (step number)")
...
class TenantFormField(TenantAwareModel):
    ...
    form_entity = models.ForeignKey(
        TenantFormEntity, on_delete=models.CASCADE, related_name="fields", help_text="Form entity this field belongs to"
    )
    ...
    field_key = models.CharField(max_length=100, help_text="Field key/name from the entity model")

    # Field type (stored explicitly to ensure correct rendering)
    field_type = models.CharField(
        max_length=30, default="text", help_text="Form field type: text, email, select, textarea, number, date, etc."
    )

    # Display settings
    is_visible = models.BooleanField(default=True, help_text="Whether this field is visible in the form")
    is_required = models.BooleanField(default=False, help_text="Override: make this field required")
    ...
    custom_label = models.CharField(
        max_length=255, blank=True, default="", help_text="Custom label (overrides default field label)"
    )

    # Custom help text override
    custom_help_text = models.CharField(max_length=500, blank=True, default="", help_text="Custom help text")

    # Default value override
    default_value = models.JSONField(null=True, blank=True, help_text="Default value for this field in this form")
    ...
    auto_populate_source_field = models.CharField(
        max_length=100, blank=True, default="", help_text="Field key from source step to use for auto-population"
    )
    ...
    validation_rules = models.JSONField(
        default=dict, blank=True, help_text="Validation rules: min_length, max_length, pattern, min, max, etc."
    )
    ...
    cascade_enabled = models.BooleanField(default=False, help_text="Enable cascading filter for this field")
...
class TenantFormRule(TenantAwareModel):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    form = models.ForeignKey(
        TenantForm, on_delete=models.CASCADE, related_name="rules", help_text="Form this rule belongs to"
    )
    ...
    name = models.CharField(max_length=255, blank=True, default="", help_text="Optional name for this rule")
    ...
    order = models.PositiveIntegerField(default=0, help_text="Order in which rules are evaluated")

    # WHEN: The condition(s) to check
    # Format: [{"field": "step1.field_name", "operator": "eq", "value": "xyz"}, ...]
    conditions = models.JSONField(default=list, help_text="Conditions to evaluate (AND logic between conditions)")
    ...
    actions = models.JSONField(default=list, help_text="Actions to perform when conditions are met")
...
class TenantWorkflow(TenantAwareModel):
    ...
    description = models.TextField(blank=True, default="", help_text="Description of what this workflow does")
    ...
    # Trigger type
    trigger_type = models.CharField(max_length=30, choices=TriggerType.choices, help_text="What triggers this workflow")
    ...
    #   }
    # }
    trigger_config = models.JSONField(default=dict, blank=True, help_text="Configuration specific to the trigger type")

    # Entity type this workflow applies to (if applicable)
    entity_type = models.CharField(
        max_length=100, blank=True, default="", help_text="Entity type this workflow operates on"
    )

    # Icon for display
    icon = models.CharField(max_length=50, blank=True, default="zap", help_text="Icon identifier for UI")
    ...
    # Execution stats
    last_run_at = models.DateTimeField(null=True, blank=True, help_text="When this workflow last ran")
    run_count = models.PositiveIntegerField(default=0, help_text="Number of times this workflow has run")
...
class TenantWorkflowCondition(TenantAwareModel):
    ...
    # Field to check
    field_path = models.CharField(max_length=255, help_text="Path to field (e.g., 'status', 'customer.name')")

    # Operator
    operator = models.CharField(max_length=20, choices=OperatorType.choices, help_text="Comparison operator")

    # Value to compare against
    compare_value = models.JSONField(null=True, blank=True, help_text="Value to compare against")

    # Order
    order = models.PositiveIntegerField(default=0, help_text="Order of condition evaluation")
...
class TenantWorkflowAction(TenantAwareModel):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    workflow = models.ForeignKey(
        TenantWorkflow, on_delete=models.CASCADE, related_name="actions", help_text="Workflow this action belongs to"
    )
    ...
    # Action type
    action_type = models.CharField(max_length=30, choices=ActionType.choices, help_text="Type of action to perform")
    ...
    config = models.JSONField(default=dict, help_text="Configuration specific to the action type")

    # Order of execution
    order = models.PositiveIntegerField(default=0, help_text="Order in which actions are executed")

    # Whether to continue on error
    continue_on_error = models.BooleanField(
        default=False, help_text="Continue executing subsequent actions if this one fails"
    )
...
class WorkflowExecutionLog(TenantAwareModel):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    workflow = models.ForeignKey(
        TenantWorkflow, on_delete=models.CASCADE, related_name="execution_logs", help_text="Workflow that was executed"
    )
    ...
    # Trigger info
    trigger_type = models.CharField(max_length=30, help_text="What triggered this execution")
    ...
    # Results
    actions_executed = models.PositiveIntegerField(default=0, help_text="Number of actions executed")
    actions_failed = models.PositiveIntegerField(default=0, help_text="Number of actions that failed")

    # Error info
    error_message = models.TextField(blank=True, default="", help_text="Error message if execution failed")

    # Detailed log
    execution_log = models.JSONField(default=list, help_text="Detailed log of each action's execution")

    # Who/what triggered it
    triggered_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, help_text="User who triggered the workflow (if manual)"
    )
...
class TenantWorkFormExecution(TenantAwareModel):
    ...
    initial_data = models.JSONField(
        default=dict, blank=True, help_text="Trigger/initial payload used to start execution"
    )
    context_data = models.JSONField(
        default=dict, blank=True, help_text="Execution context snapshot (variables, errors, outputs)"
    )
    audit_trail = models.JSONField(
        default=list, blank=True, help_text="Chronological log of node transitions and actions"
    )
...
class WorkflowDeadLetter(TenantAwareModel):
    ...
    status = models.CharField(
        max_length=20, choices=WorkflowDeadLetterStatus.choices, default=WorkflowDeadLetterStatus.PENDING
    )
...
class FormSubmission(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    form = models.ForeignKey(
        TenantForm, on_delete=models.CASCADE, related_name="submissions", help_text="The form being submitted"
    )
    ...
    data = models.JSONField(default=dict, blank=True, help_text="All field values: { step_id: { field_key: value } }")

    # Snapshot of form structure at creation time (for versioning)
    form_snapshot = models.JSONField(
        default=dict, blank=True, help_text="Snapshot of form structure at submission creation"
    )
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    completed_at = models.DateTimeField(null=True, blank=True, help_text="Timestamp when submission was completed")
    ...
    def progress(self):
        ...
        completed = self.step_submissions.filter(status=StepSubmissionStatus.COMPLETED).count()
...
class FormStepSubmission(models.Model):
    ...
    submission = models.ForeignKey(
        FormSubmission, on_delete=models.CASCADE, related_name="step_submissions", help_text="Parent form submission"
    )
    ...
    step = models.ForeignKey(
        TenantFormEntity, on_delete=models.CASCADE, related_name="step_submissions", help_text="The form step/entity"
    )
    ...
    data = models.JSONField(default=dict, blank=True, help_text="Field values for this step: { field_key: value }")

    # Completion tracking
    completed_at = models.DateTimeField(null=True, blank=True, help_text="Timestamp when step was marked complete")
...
class FormSubmissionFile(models.Model):
    ...
    field_key = models.CharField(max_length=100, help_text="The field key this file is associated with")

    # File storage
    file = models.FileField(upload_to=form_submission_upload_path, help_text="The uploaded file")
    original_name = models.CharField(max_length=255, help_text="Original filename as uploaded")
...
class FormSubmissionEvent(models.Model):
    ...
    # Event details
    event_type = models.CharField(max_length=30, choices=FormSubmissionEventType.choices, help_text="Type of event")
    step_id = models.UUIDField(null=True, blank=True, help_text="Step ID if event is step-related")
    field_key = models.CharField(
        max_length=100, blank=True, default="", help_text="Field key if event is field-related"
    )

    # Event metadata
    metadata = models.JSONField(
        default=dict, blank=True, help_text="Additional event data (error messages, field values, etc.)"
    )

    # Timing
    created_at = models.DateTimeField(auto_now_add=True)
    duration_ms = models.PositiveIntegerField(
        null=True, blank=True, help_text="Duration in milliseconds (for step/form completion)"
    )
...
class FormStatusHistory(models.Model):
    ...
    to_status = models.CharField(
        max_length=20, choices=FormSubmissionStatus.choices, help_text="New status after this change"
    )
    ...
    comment = models.TextField(blank=True, default="", help_text="Optional reason or comment for the status change")
...
class StepAssignment(models.Model):
    ...
    step = models.ForeignKey(
        TenantFormEntity, on_delete=models.CASCADE, related_name="assignments", help_text="The step being assigned"
    )
    ...
    assignment_type = models.CharField(
        max_length=20, choices=AssignmentType.choices, default=AssignmentType.USER, help_text="Type of assignment"
    )
    ...
    # Assignment rules
    is_required = models.BooleanField(default=True, help_text="Whether this step must be completed (vs. skippable)")
    due_days = models.PositiveIntegerField(
        null=True, blank=True, help_text="Number of days allowed to complete this step (SLA)"
    )
    ...
    def __str__(self):
        step_label = getattr(self.step, "step_name", None) or getattr(self.step, "name", None) or str(self.step)
...
class UserNotification(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="notifications", help_text="User to receive this notification"
    )
    ...
    notification_type = models.CharField(
        max_length=30, choices=NotificationType.choices, help_text="Type of notification"
    )
    title = models.CharField(max_length=200, help_text="Notification title (short, e.g., 'New Task Assigned')")
    ...
    entity_id = models.UUIDField(null=True, blank=True, help_text="UUID of the related entity")
    action_url = models.CharField(
        max_length=500, blank=True, default="", help_text="URL to navigate to when notification is clicked"
    )
    ...
    is_dismissed = models.BooleanField(default=False, help_text="Whether the user dismissed this notification")
    ...
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="When this notification should auto-expire (for cleanup)"
    )
...
class UserNotificationPreferences(models.Model):
    ...
    notifications_enabled = models.BooleanField(
        default=True, help_text="Master switch to enable/disable all notifications"
    )
    ...
    type_preferences = models.JSONField(default=dict, blank=True, help_text="Per-type delivery preferences")
    ...
    weekly_digest_enabled = models.BooleanField(default=False, help_text="Receive a weekly digest email")
    ...
    def get_defaults(cls) -> dict[str, list[str]]:
        ...
        return {str(nt.value): ([in_app, email] if nt in email_enabled_types else [in_app]) for nt in NotificationType}
