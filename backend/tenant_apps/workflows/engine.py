...
class SendEmailExecutor(ActionExecutor):
    ...
    def _resolve_recipient(self, action: TenantWorkflowAction, config: Dict, context: Dict) -> Dict | None:
...
class SendNotificationExecutor(ActionExecutor):
    ...
    def execute(self, action: TenantWorkflowAction, context: Dict) -> Dict:
        ...
        return {"success": True, "message": "Notification queued", "data": {"title": title, "message": message}}
...
class SetFieldValueExecutor(ActionExecutor):
    ...
    def execute(self, action: TenantWorkflowAction, context: Dict) -> Dict:
        ...
        return {"success": True, "message": f"Set {field} = {value}", "data": {"field": field, "value": value}}
    ...
    def execute(self, action: TenantWorkflowAction, context: Dict) -> Dict:
        ...
        try:
            workflow = TenantWorkflow.objects.get(
                id=workflow_id, tenant=context.get("tenant"), status=WorkflowStatus.ACTIVE
            )
...
class WorkflowEngine:
    ...
    def find_workflows_for_trigger(self, tenant, trigger_type: str, entity_type: str = None) -> List[TenantWorkflow]:
        """Find active workflows that match the trigger criteria."""
        qs = TenantWorkflow.objects.filter(tenant=tenant, status=WorkflowStatus.ACTIVE, trigger_type=trigger_type)
    ...
    def execute(
        self, workflow: TenantWorkflow, record_data: Dict, triggered_by=None, trigger_type: str = "manual"
    ) -> Dict:
        """
        ...
        try:
            ...
            if not ConditionEvaluator.evaluate_all(workflow, record_data):
                ...
                return {"success": True, "skipped": True, "message": "Conditions not met"}
            ...
            for action in workflow.actions.order_by("order"):
                ...
                try:
                    ...
                    execution_log.append(
                        {"action": action.action_type, "order": action.order, "status": "error", "message": str(e)}
                    )
            ...
            return {"success": False, "status": "failed", "error": str(e), "execution_id": str(log.id)}

    def trigger_for_record(
        self, tenant, entity_type: str, trigger_type: str, record_data: Dict, user=None
    ) -> List[Dict]:
        """
        ...
        for workflow in workflows:
            ...
            results.append({"workflow_id": str(workflow.id), "workflow_name": workflow.name, **result})
...
def trigger_scheduled_workflows():
    ...
    workflows = TenantWorkflow.objects.filter(status=WorkflowStatus.ACTIVE, trigger_type=TriggerType.SCHEDULED)
...
def on_record_created(sender, instance, tenant, entity_type, **kwargs):
    ...
    results = engine.trigger_for_record(
        tenant=tenant, entity_type=entity_type, trigger_type=TriggerType.RECORD_CREATED, record_data=record_data
    )
...
def on_record_updated(sender, instance, tenant, entity_type, changed_fields=None, **kwargs):
    ...
    results = engine.trigger_for_record(
        tenant=tenant, entity_type=entity_type, trigger_type=TriggerType.RECORD_UPDATED, record_data=record_data
    )
