...
from tenant_apps.plants.models import Plant, PlantAssociatedMasterProduct, PlantProteinOffered, PlantProteinTested
from tenant_apps.products.models import MasterProduct

from apps.core.models import PhoneTypeChoices, Protein
    ...
    def to_representation(self, instance):
        ...
        if tenant:
            data["proteins_offered"] = [
                link.protein.name
                for link in instance.protein_offered_links.filter(tenant=tenant)
                .select_related("protein")
                .order_by("protein__name")
            ]
            data["proteins_tested"] = [
                link.protein.name
                for link in instance.protein_tested_links.filter(tenant=tenant)
                .select_related("protein")
                .order_by("protein__name")
            ]
        else:
