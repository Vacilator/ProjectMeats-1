...
class ActivityLog(TenantAwareModel):
    ...
    content_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE, null=True, blank=True, help_text="Type of entity this log relates to"
    )
    object_id = models.PositiveIntegerField(null=True, blank=True, help_text="ID of the related entity")
    content_object = GenericForeignKey("content_type", "object_id")

    # Alternative simple approach (denormalized for faster queries)
    entity_type = models.CharField(
        max_length=50, choices=EntityTypeChoices.choices, help_text="Entity type (Supplier, Customer, Order, etc.)"
    )
    entity_id = models.PositiveIntegerField(help_text="ID of the entity")

    # Activity content
    title = models.CharField(max_length=255, blank=True, default="", help_text="Optional title/subject of the activity")
    content = models.TextField(help_text="Activity note content (supports multiline text)")
    ...
    is_pinned = models.BooleanField(default=False, help_text="Pin important notes to top of feed")

    # Tags for categorization (optional)
    tags = models.CharField(
        max_length=255, blank=True, default="", help_text="Comma-separated tags (e.g., 'call, follow-up, urgent')"
    )
...
class ScheduledCall(TenantAwareModel):
    ...
    entity_type = models.CharField(
        max_length=50, choices=EntityTypeChoices.choices, help_text="Entity type for this call"
    )
    entity_id = models.PositiveIntegerField(help_text="ID of the related entity")

    # Call details
    title = models.CharField(max_length=255, help_text="Call subject/title")
    description = models.TextField(blank=True, default="", help_text="Call notes/agenda")

    # Scheduling
    scheduled_for = models.DateTimeField(help_text="When is this call scheduled?")
    duration_minutes = models.PositiveIntegerField(default=30, help_text="Expected call duration in minutes")

    # Status
    is_completed = models.BooleanField(default=False, help_text="Has this call been completed?")
    completed_at = models.DateTimeField(null=True, blank=True, help_text="When was this call completed?")
...
class UserWorkspaceLayout(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="workspace_layout", help_text="User this layout belongs to"
    )

    # Layout data stored as JSON
    layout = models.JSONField(default=list, help_text="Widget layout positions (react-grid-layout format)")
    ...
    version = models.PositiveIntegerField(default=1, help_text="Layout schema version for future migrations")
