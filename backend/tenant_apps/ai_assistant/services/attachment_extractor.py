...
def build_combined_attachment_text(extracted_attachments: list[dict[str, Any]]) -> str:
    ...
    Each section is clearly tagged with its source (attachment filename) to
    distinguish from the email body content in downstream AI extraction.
    """
    sections: list[str] = []
    for idx, att in enumerate(extracted_attachments, 1):
        text = att.get("extracted_text", "")
        if not text:
            continue
        name = att.get("name", f"attachment_{idx}")
        content_type = att.get("content_type", "unknown")
        sections.append(f"--- Source: attachment_{idx} | File: {name} | Type: {content_type} ---\n{text}")
