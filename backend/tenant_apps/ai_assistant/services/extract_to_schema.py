...
def get_extract_document(*, document_id: Any, tenant: Any, user: Any) -> AIDocument:
    document = (
        AIDocument.objects.select_related('tenant', 'owner')
        .filter(id=document_id, tenant=tenant, owner=user)
        .first()
    )
    if not document:
...
def extract_document_to_schema(*, document: AIDocument, entity_type: str, tenant: Any, user: Any) -> ExtractedSchemaDraft:
