...
def dispatch_webhook_payload(self, webhook_id: int, tenant_id: str, event_type: str, payload: Dict[str, Any]):
    ...
    try:
        webhook = TenantWebhook.objects.select_related("tenant").filter(id=webhook_id, tenant_id=tenant_id).first()
        if not webhook:
...
def process_settlement_event(self, event_id: int, tenant_id: str):
    ...
    try:
        event = SettlementEvent.objects.select_related("source").filter(id=event_id, tenant_id=tenant_id).first()
        if not event:
        ...
        event.normalized_payload = (
            normalized_payload if isinstance(normalized_payload, dict) else {"body": normalized_payload}
        )
