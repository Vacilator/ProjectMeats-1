...
class EmailIngestionService:
    ...
    def _save_email_log(
        ...
        # Log to tenant activity feed with explicit attachment count
        try:
            ...
            if att_count > 0:
                description = (
                    f'Email + {att_count} attachment{"s" if att_count != 1 else ""} '
                    f"processed from {sender_email}: {email_log.subject}"
                )
            else:
                description = f"Email processed from {sender_email}: {email_log.subject}"
