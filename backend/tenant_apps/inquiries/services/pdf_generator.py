...
class InquiryPDFGenerator:
    ...
    def _build_header(self):
        ...
        # Quote subtitle
        elements.append(Paragraph(f"Quote #{self.inquiry.inquiry_number}", self.styles["Heading2"]))
    ...
    def _build_info_section(self):
        ...
        if self.inquiry.contact_name:
            right_col.append(Paragraph(f"<b>{self.inquiry.contact_name}</b>", self.styles["InfoValue"]))

        if self.inquiry.contact_company:
            right_col.append(Paragraph(self.inquiry.contact_company, self.styles["InfoValue"]))

        if self.inquiry.contact_position:
            right_col.append(Paragraph(self.inquiry.contact_position, self.styles["InfoValue"]))

        if self.inquiry.contact_email:
            right_col.append(Paragraph(self.inquiry.contact_email, self.styles["InfoValue"]))

        if self.inquiry.contact_phone:
            right_col.append(Paragraph(self.inquiry.contact_phone, self.styles["InfoValue"]))

        # Create two-column table
        info_table = Table([[left_col, right_col]], colWidths=[3.5 * inch, 3.5 * inch])
        info_table.setStyle(
            TableStyle(
                [
                    ("VALIGN", (0, 0), (-1, -1), "TOP"),
