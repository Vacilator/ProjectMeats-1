...
class Claim(TenantAwareModel):
    ...
    claim_number = models.CharField(max_length=100, unique=True, help_text="Unique claim number/reference")
    ...
    status = models.CharField(
        max_length=20, choices=ClaimStatus.choices, default=ClaimStatus.PENDING, help_text="Current status of the claim"
    )
    ...
    invoice = models.ForeignKey(
        Invoice, on_delete=models.SET_NULL, null=True, blank=True, related_name="claims", help_text="Related invoice"
    )

    # Claim details
    reason = models.TextField(help_text="Reason for the claim (e.g., damaged goods, incorrect quantity, quality issue)")
    description = models.TextField(blank=True, default="", help_text="Detailed description of the claim")

    # Financial
    claimed_amount = models.DecimalField(
        max_digits=10, decimal_places=2, default=Decimal("0.00"), help_text="Amount being claimed"
    )
    approved_amount = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Approved amount (may differ from claimed)"
    )
    settled_amount = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Actual amount paid/settled"
    )

    # Dates
    claim_date = models.DateField(help_text="Date claim was filed")
    resolution_date = models.DateField(null=True, blank=True, help_text="Date claim was resolved/settled")

    # Resolution
    resolution_notes = models.TextField(blank=True, default="", help_text="Notes on how claim was resolved")
...
class PaymentTransaction(TenantAwareModel):
    ...
    payment_method = models.CharField(
        max_length=20, choices=PaymentMethod.choices, default=PaymentMethod.CHECK, help_text="Method of payment"
    )
    reference_number = models.CharField(
        max_length=100, blank=True, default="", help_text="Check number, wire confirmation, or transaction ID"
    )
    notes = models.TextField(blank=True, default="", help_text="Additional notes about this payment")
    ...
    def _update_sales_order_payment_status(self):
        ...
        so = self.sales_order
        total_paid = sum(payment.amount for payment in so.payments.all())
    ...
    def _update_invoice_payment_status(self):
        ...
        inv = self.invoice
        total_paid = sum(payment.amount for payment in inv.payments.all())
