...
from tenant_apps.invoices.models import Claim, Invoice, PaymentTransaction
from tenant_apps.invoices.serializers import ClaimSerializer, InvoiceSerializer, PaymentTransactionSerializer
...
class InvoiceViewSet(OperationalDocumentActionsMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Invoice.all_objects.filter(tenant=self.request.tenant)
            if (include_deleted and is_admin)
            else Invoice.objects.filter(tenant=self.request.tenant)
        )
