...
class DeploymentTester:
    ...
    def __init__(self, environment: str = "development"):
        ...
        if environment == "staging":
            self.log(
                "⚠️  NOTE: 'staging' refers to UAT environment "
                "(uat.meatscentral.com)",
                "WARNING",
            )
            self.log(
                "   staging.meatscentral.com is DEPRECATED - UAT is the "
                "active middle environment",
                "WARNING",
            )

        self.test_results = {"passed": 0, "failed": 0, "warnings": 0, "tests": []}
    ...
    def run_deployment_tests(self, full_test: bool = False) -> bool:
        ...
        self.log("=" * 60, "HEADER")
        self.log(
             f"ProjectMeats3 Deployment Testing - "
             f"{self.environment.upper()} Environment",
             "HEADER",
         )
        self.log("=" * 60, "HEADER")
        ...
        self.log(
             ...
             "ERROR" if self.test_results["failed"] > 0 else "INFO",
        )
        self.log(
             f"Warnings: {self.test_results['warnings']}",
             "WARNING" if self.test_results["warnings"] > 0 else "INFO",
        )
...
Examples:
  ...
  python test_deployment.py --validate-only
    ...
    parser.add_argument(
        ...
        default="development",
        help=(
            'Environment to test (default: development). NOTE: Use "staging" '
            "for UAT environment - staging.meatscentral.com is deprecated."
        ),
    )
