...
| TD-BE-002 | Backend | First-batch tenant idempotency shipped, but remaining mutating APIs still lack explicit idempotency coverage or exemption guidance | High | M | `backend/apps/core/services/idempotency.py`, `backend/tenant_apps/ai_assistant/views.py`, `backend/apps/system/workform_views.py` | ~~Extend to remaining mutating APIs~~ @idempotent_create wired to all 5 trade entity creates (PR #5520). AI chat/feedback endpoints exempt (non-critical, high-volume) | EH-02 | ✅ Partially Resolved |
