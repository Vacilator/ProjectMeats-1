...
- **2026-05-01** — Phase 14 governance execution is in review on `GA-03.2 seven-year-archive-command` (PR #4823), which adds the dry-run-first `archive_historical_records` management command, tenant-scoped `ArchiveBatch` / `ArchiveRecordSnapshot` / `ArchiveLegalHold` evidence tables with additive RLS policies, legal-hold enforcement for direct and parent-linked records, and updated retention guardrails/runbook coverage. Merge this PR to promote `GA-03.3 pii-redaction-for-logging-and-sentry` to the next ready ticket.
