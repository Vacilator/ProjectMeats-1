...
- **2026-05-13** — AI inbox/trade console error hardening: removed the plain-HTTP `/ws/ai/inbox/` preflight from `AIAgentWidget` so the widget now relies on the real native websocket upgrade path instead of surfacing the backend’s intentional 426 probe response; added feedback-backed ai-document placeholder retrieval plus hydration short-circuits so pending review and chat surfaces stop hammering missing `AIDocument` rows; and fixed `/api/v1/trades/proposals/` to read the current Inquiry schema while pausing cross-tab auto-polling through a shared proposals circuit breaker when the backend is degraded. Validation used focused frontend Vitest coverage for `AIAgentWidget`, `AITradeProposals`, and `aiService.documentHydration`, `npm -C frontend run type-check`, focused `eslint` on the touched frontend files, and `DATABASE_URL='' DJANGO_SETTINGS_MODULE=projectmeats.settings.test TEST_DB_NAME=test_projectmeats_incident_console_errors python backend/manage.py test tenant_apps.ai_assistant.tests.test_models tenant_apps.inquiries.tests_trade_session tenant_apps.ai_assistant.test_inbox_websocket --noinput`. (PR: #5342)
- **2026-05-13** — PR-log completion + Phase 38 wording cleanup: backfilled the missing shipped evidence for the large entity/document field-parity batch and the AICommandCenter modal-wrapper follow-up, and tightened the remaining Phase 38 ticket wording so the next recovery lane is described from the now-shipped AUTO-38.1/AUTO-38.2 baseline. Validation used `bash scripts/verify_golden_state.sh` and `bash .github/scripts/check_infrastructure.sh`. (PR: #5338)
- **2026-05-13** — Canonical entity/document field parity across schema-driven create/edit surfaces: expanded shared business/payment/product/logistics enums plus additive tenant model/serializer/migration coverage for carriers, suppliers, customers, sales orders, invoices, and purchase-order/carrier-PO flows; removed simplified supplier/customer/sales-order schema shortcuts so the universal form surfaces expose the spreadsheet-driven grouped field sets; extended AI draft + sequential entity helpers to cover the broader field map; and restored legacy system-product choice aliases so the backend contract stayed compatible while the new canonical spreadsheet values became valid. Validation used frontend type-check, focused frontend tests, `npm -C frontend run lint:colors`, `npm -C frontend run lint:render-stability`, `python manage.py check`, `python manage.py makemigrations --check`, and focused backend regression coverage. (PR: #5333)
- **2026-05-13** — AICommandCenter modal-body wrapper hardening: preserved the already-shipped operator-surface inline-style cleanup while replacing the detail-modal padding override with an inner wrapper so the complex-form modal path satisfies the AI PR Gatekeeper air-gap rule; the follow-up cleared the full PR validation suite on `development`. (PR: #5337)
- **2026-05-13** — AUTO-38.3 recoverable sync retry/reconnect/progress CTA hardening: extended the integrations auto-sync/manual-sync/tool failure contract with machine-readable retry/reconnect actions plus coarse progress/summary metadata, kept queued/in-flight task status fail-closed by tenant, and moved frontend sync lifecycle ownership into `AIInboxSyncContext` so `AIAgentWidget` and `AICommandCenter` now show explicit retry/reconnect CTAs and long-sync progress instead of silent timeout-shaped success. Validation used `cd backend && python manage.py test apps.integrations tenant_apps.ai_assistant --noinput`, `cd frontend && NODE_OPTIONS=--max-old-space-size=8192 npx tsc --noEmit`, focused `eslint --quiet` on the touched sync files/tests, focused `vitest` for `AICommandCenter` / `AIAgentWidget` / `AIInboxSyncContext`, and `cd frontend && npm run test:ci`. (PR: #5339)
- **2026-05-13** — Canonical Phase 38 backlog sync after AUTO-38.2 merge: marked `AUTO-38.2 ai-inbox-provenance-parse-status-and-retryability-badges` shipped on `development` after PR #5332, promoted `AUTO-38.3 recoverable-sync-retry-reconnect-and-progress-cta` to the sole `Ready` ticket in `MASTER_PLAN.md` and `.github/EPIC_TICKETS.md`, and kept `AUTO-38.4` explicitly blocked behind the recovery lane so autonomous continuation points at the correct next batch again. Validation used `bash scripts/verify_golden_state.sh` and `bash .github/scripts/check_infrastructure.sh`. (PR: #5336)
- **2026-05-13** — Supplier → Plant schema-cascade hardening: published supplier/plant entity hierarchy through the shared navigation context, taught the shared entity-create surface to auto-cascade and lock matching parent FK values from that hierarchy instead of page-local `initialValues`, aligned supplier record and supplier-plants “New Plant” entrypoints to the shared context path, and added additive `Plant.site_code` model/serializer/migration coverage so the metadata-driven plant form exposes the missing backend field. Validation used `python backend/manage.py makemigrations --check`, `python backend/manage.py test tenant_apps.plants --noinput`, targeted `eslint --quiet` on the touched frontend files, and focused jsdom Vitest coverage for `NavigationContext`, `Breadcrumb`, `Plants`, and `Plants.edit`. (PR: #5335)
- **2026-05-12** — AUTO-38.2 AI Inbox provenance/parse-status/retryability badges: extended `frontend/src/services/aiService.ts` so pending review items hydrate document audit metadata from `document_id` when the review payload omits it; rendered shared `DocumentAuditBadges` in `frontend/src/pages/AICommandCenter.tsx` and `frontend/src/components/AIAssistant/AIDraftReviewModal.tsx`; added focused regression coverage for service hydration, AI Inbox list/modal badge rendering, modal render stability, and the previously unrelated `AITradeProposals.test.tsx` AntD message test blocker so `npm -C frontend run test:ci` completes cleanly. Validation used `npm -C frontend run verify-standards`, targeted vitest coverage for the touched AI Inbox/service files, and `npm -C frontend run test:ci`. (PR: #5332)
- **2026-05-12** — Canonical Phase 38 backlog sync after AUTO-38.1 merge: marked `AUTO-38.1 backend-email-ingest-error-contract-and-status-metadata` shipped on `development` after PR #5330 and promoted `AUTO-38.2 ai-inbox-provenance-parse-status-and-retryability-badges` to the sole `Ready` ticket in `MASTER_PLAN.md` and `.github/EPIC_TICKETS.md`, with downstream blocker/dependency text updated accordingly. Validation used `bash scripts/verify_golden_state.sh` and `bash .github/scripts/check_infrastructure.sh`. (PR: #5331)
- **2026-05-12** — AUTO-38.1 backend email ingest error contract + status metadata: added the shared `backend/apps/integrations/email_failure_contract.py` helper plus additive `EmailLog.failure_code` / `status_metadata` fields and migration so email sync/log surfaces persist stable auth/decrypt/network/quota/processing failure semantics; updated integrations sync/log endpoints and AI email tool/executor payloads to expose structured retryability/category metadata without raw exception matching; added focused backend regressions for sync reconnect/network cases and AI email tool error payloads; and refreshed `manifests/openapi/openapi-schema.baseline.json` to match the new integrations contract. Validation used `python manage.py makemigrations integrations`, `python manage.py makemigrations --check`, focused + package-level backend tests for `apps.integrations` / `tenant_apps.ai_assistant`, `python manage.py spectacular --format openapi-json --verbosity 0`, `python scripts/check_openapi_backcompat.py`, and `python manage.py spectacular --validate --file /tmp/projectmeats-openapi.yaml`. (PR: #5330)
- **2026-05-12** — Seeded Phase 38 email automation reliability + operator diagnostics as the next canonical execution lane after Phase 37 completion. Added `AUTO-38.1` through `AUTO-38.4` to `.github/EPIC_TICKETS.md` with exactly one `Ready` ticket (`AUTO-38.1 backend-email-ingest-error-contract-and-status-metadata`) and updated `MASTER_PLAN.md` so the canonical focus now points to the new reliability/diagnostics lane instead of the completed operator-surface cleanup batch. (PR: #5329)
- **2026-05-12** — Supplier/customer create-modal stability hardening: stabilized the shared create-modal surface around supplier/customer child-entry flows by keeping the plant/location create forms on the standard `EntityFormSurface` modal path, memoizing the supplier/customer record breadcrumb/create wiring, and adding focused regression coverage for supplier-scoped **New Plant** / **Add Plant** defaults. Validation used focused frontend tests, `npm -C frontend run verify-standards`, a Chromium production-preview entity-form smoke, and the full frontend `test:ci` suite. (PR: #5327)
- **2026-05-12** — UX-37.5 AICommandCenter shortcut/deep-link/confidence regression hardening: added focused `AICommandCenter.test.tsx` coverage for Alt+1-4 tab switching, input-focus shortcut guards, `?item=` deep-link modal open/close URL behavior, overview AI Inbox preview limits, and exact confidence-threshold boundary renders. The new close-path coverage also exposed and fixed a draft-review deep-link reopen race in `AICommandCenter.tsx` by suppressing reopen until the `item` param is cleared from the URL. Validation used `npm -C frontend run verify-standards`, focused `AICommandCenter.test.tsx`, and the full frontend `test:ci` suite. (PR: #5326)
- **2026-05-12** — UX-37.4 workspace terminology alignment: aligned the surviving secondary-workspace copy with the Command Center-first model by renaming the visible workspace shell to “Workspace Dashboard” across Header onboarding, Cockpit page titles, onboarding empty states, Cockpit tour copy, widget-surface CTAs, and cockpit record breadcrumbs while preserving existing `/cockpit/*` routes and tour logic. Validation used `npm -C frontend run verify-standards`, focused `Header.test.tsx` / `CockpitWelcomeEmptyState.test.tsx` / `CockpitDashboard.test.tsx`, and the full frontend `test:ci` suite. (PR: #5325)
- **2026-05-12** — UX-37.3 Cockpit workspace search/catalog cleanup: replaced the Cockpit SmartSearch forwarding stub with static Command Center search guidance, removed `AILearningMetricsWidget` from new widget-catalog additions, and kept saved layouts render-safe for existing AI Learning Metrics widgets. Validation used `npm -C frontend run verify-standards`, focused `CockpitDashboard.test.tsx`, and the full frontend `test:ci` suite. (PR: #5324)
- **2026-05-12** — Copilot session shipping-discipline instruction hardening: updated the session-loaded Copilot CLI instruction sources so they now explicitly state “ALWAYS do the following for every batch of changes made: create a new branch, create a new PR, and merge to development,” reinforcing the existing branch → PR → merge workflow before future sessions start work. (PR: #5323)
- **2026-05-11** — UX-37.2 AICommandCenter confidence/intent/record inline-style extraction: finished the remaining operator-surface cleanup in `frontend/src/pages/AICommandCenter.tsx` by converting the header title/search/refresh wrappers, AI Inbox badge + intent/confidence pills, pipeline count text, and detail-modal intent/contact/open-record/process-flow wrappers into named styled-components. Validation used `npm -C frontend run verify-standards`, focused `AICommandCenter.test.tsx`, and the full frontend `test:ci` suite. (PR: #5321)
- **2026-05-11** — Dev deploy-plan hardening + supplier New Plant React #185 fix: replaced the second `actions/checkout`-dependent diff probe in `main-pipeline.yml` with GitHub API file detection plus safe full-deploy fallback so transient GitHub fetch failures no longer abort development routing, and aligned the supplier record New Plant modal with the known-good plant-create defaults while stabilizing the child-create tab callbacks. Added focused `UniversalEntityRecordPage` coverage plus a Chromium production-preview smoke for the supplier child modal path alongside the existing plant create/edit guardrails. (PR: #5320)
- **2026-05-11** — UX-37.1 AICommandCenter trade-table + detail-modal inline-style extraction: converted the trade-table typography/tags/action button cluster plus the trade-detail modal tags, metadata rows, source/inquiry links, and footer controls into named styled-components, reducing inline-style drift inside the primary operator surface without changing query behavior or user-facing copy. Validation used frontend type-check, file-scoped `eslint` on `AICommandCenter.tsx`, focused `AICommandCenter.test.tsx` coverage, and the full frontend `test:ci` suite. (PR: #5319)
- **2026-05-11** — Seeded Phase 37 operator surface completion as the next canonical frontend simplification lane after Phase 36 completion. Added `UX-37.1` through `UX-37.5` to `.github/EPIC_TICKETS.md` with exactly one `Ready` ticket (`UX-37.1 aicommandcenter-trade-table-and-modal-inline-style-extraction`), updated `MASTER_PLAN.md` to make Phase 37 the active focus, and aligned `ROADMAP.md` / `UI_ROADMAP.md` as reference-only pointers to that canonical lane. (PR: #5318)
- **2026-05-11** — UX-36.3 ProcessCockpit legacy retirement + secondary copy alignment: deleted the dead `ProcessCockpitPage` and `ProcessMonitor` frontend surfaces plus their dead-only regression files, preserved the live legacy redirects into Command Center, and aligned the surviving Cockpit workspace labels, onboarding copy, header menu entry, and entity breadcrumbs with the Command Center-first operator model. Added focused regression coverage on the surviving cockpit/header copy surfaces while keeping the full frontend suite green. (PR: #5317)
- **2026-05-11** — UX-36.2 WorkForms Monitoring single drill-in surface: removed the embedded legacy `ProcessMonitor` queue shell from `frontend/src/pages/WorkForms/Monitoring.tsx`, kept the page focused on execution analytics + active runs, and added explicit follow-through actions back to Command Center, WorkForms History, and WorkForms Catalog. Added focused regression coverage proving the legacy submission queue is no longer embedded and that the new history/catalog navigation paths remain wired. (PR: #5316)
- **2026-05-11** — UX-36.1 Command Center four-section compression + Workflows demotion: removed the top-level Workflows tab from `frontend/src/pages/AICommandCenter.tsx`, added explicit execution-monitoring drill-in entrypoints, updated keyboard/tab affordances to the four-section model, and redirected surviving `/command-center?tab=workflows...` URLs into `/workforms/monitoring` while preserving the rest of the query string. Added focused regression coverage for the removed-tab redirect, the new execution-monitoring CTA, and the updated keyboard-shortcut hint. (PR: #5315)
- **2026-05-11** — Seeded Phase 36 operator execution surface simplification as the next canonical execution lane after Phase 35 completion. Added `UX-36.1` through `UX-36.3` to `.github/EPIC_TICKETS.md` with exactly one `Ready` ticket (`UX-36.1 command-center-four-section-ia-and-workflows-demotion`), updated `MASTER_PLAN.md` to make Phase 36 the active focus, and aligned `ROADMAP.md` / `UI_ROADMAP.md` as reference-only pointers to that canonical lane. (PR: #5314)
- **2026-05-11** — UX-35.5 action-required/process-ops consolidation + copy polish: consolidated the remaining operator-home drift under Command Center by adding wildcard legacy route redirects for action-required and pipeline surfaces, reframing WorkForms Monitoring as the secondary execution drill-in, removing duplicated email/action-required panels from that monitoring queue, and aligning cockpit/workspace copy plus dashboard return paths to the Command Center-first model. Added regression coverage for the new wildcard redirects, monitoring guidance, Command Center empty-state drill-in, updated header onboarding/dashboard labels, and the refreshed ingestion/workspace copy. (PR: #5313)
- **2026-05-11** — UX-35.4 cockpit dashboard top-level surface reduction: removed standalone AI learning metrics from the default populated Cockpit landing surface, kept that capability reachable through widget customization, and narrowed the primary landing path to the intended minimal search/chips/customize/grid model while preserving blank-state onboarding, cockpit search handoff, and saved-layout behavior. Added regression coverage proving the simplified populated surface stays clean and that AI learning metrics remains reachable through customize-mode catalog flow. (PR: #5312)
- **2026-05-11** — UX-35.3 shared operator shell extraction: added `frontend/src/components/Shared/OperatorShell.tsx` as the additive shared chrome layer for operator pages, then refactored `AICommandCenter` and the surviving `CockpitDashboard` to consume shared header/action-row/tab/inset primitives without moving search ownership, query logic, or widget/dashboard state. Added Command Center regression coverage proving tab changes preserve the canonical `q` URL state and retained the cockpit search-handoff / widget-grid coverage around the shared-shell adoption. (PR: #5311)
- **2026-05-11** — UX-35.2 canonical search entrypoint unification: introduced a shared canonical search helper so the Header, Command Center, and legacy cockpit handoff all converge on `/command-center?q=...` while preserving `tab` / `item` state where relevant; moved Header submission off stale `/cockpit?q=...` routes; redirected legacy `/cockpit?q=...` entry into Command Center; made `AICommandCenter` read/write the canonical `q` URL param instead of local-only search state; and stopped `CommandPalette` from routing canonical entity hits back through Cockpit. Added focused regression coverage for the shared helper, Header preservation behavior, Command Center URL syncing, CommandPalette canonical-entity navigation, and cockpit search handoff. (PR: #5310)
- **2026-05-11** — UX-35.1 Command Center IA + route canonicalization: made `/command-center` the single primary operator home by redirecting `/` and the `/cockpit` index to Command Center, aligning top-level navigation and the `g d` shortcut to that canonical destination, reframing Cockpit as a secondary operations workspace, and preserving cockpit onboarding/tour continuity through `/cockpit/dashboard?tour=cockpit`. Added focused navigation/shortcut regression coverage and stabilized the related Header/SettlementQueue tests under the frontend Vitest config. (PR: #5309)
- **2026-05-11** — Seeded Phase 35 frontend surface simplification as the next canonical execution lane now that the prior executable backlog is complete. Added `UX-35.1` through `UX-35.5` to `.github/EPIC_TICKETS.md` with exactly one `Ready` ticket (`UX-35.1 command-center-information-architecture-and-route-canonicalization`), updated `MASTER_PLAN.md` to make Phase 35 the current focus, converted stale "next ready"/"active first unchecked" language in older planning snapshots to historical wording, and added reference-only roadmap pointers so future Cockpit/Command Center work is anchored to canonical docs instead of legacy roadmap text. (PR: #5308)
- **2026-05-11** — EPIC_TICKETS ready-state drift correction: converted the historical Sprint Execution Packages section in `.github/EPIC_TICKETS.md` from future-tense planning language to shipped-history wording so already-completed packages no longer present a false `Ready (P0)` continuation target. This restores deterministic autonomous backlog pickup when no unchecked ticket remains. (PR: #5307)
- **2026-05-11** — AI widget websocket hardening: added a backend HTTP probe on `/ws/ai/inbox/`, switched the backend container to ASGI/Daphne, and wired `/ws/` through deploy nginx, host nginx, and Vite so the AI Inbox socket resolves through one consistent runtime path. Hardened `AIAgentWidget` to fail closed on SPA fallback/redirect responses without opening doomed native websockets, while preserving bounded retry/backoff behavior. Added backend websocket-probe coverage, focused widget unit coverage, and a browser smoke that fails on raw websocket console noise. (PR: #5306)
- **2026-05-11** — Plant + child entity React #185 hardening: stabilized Plant create/edit and adjacent child-entry flows by removing live `searchParams` dependencies from Plant/Location context loaders, converting Plant detail edit callbacks to stable `useCallback` handlers, and replacing query-result-object `refetch()` callback dependencies with stable `queryClient.invalidateQueries()` refresh paths across Contacts, Customers, Suppliers, Carriers, Freight Orders, and Inquiries. Added Plant create regression coverage in `EntityFormSurface.test.tsx` plus create/edit production-preview browser smoke in `entity-form-surface-smoke.spec.ts`. (PR: #5303)
- **2026-05-08** — React #185 render loop comprehensive sweep: stabilized all EntityFormSurface callback patterns across 11 files. Engine-level fixes: DynamicFormEngine.tsx onValuesChange ref pattern, UniversalEntityForm.tsx onSubmittingChange ref pattern. Page-level fixes: Suppliers, Customers, Contacts, Carriers, FreightOrders, Inquiries (4-6 useCallback handlers each + ref-based cache). Component-level: UnifiedEntityTable, CustomerDetailView (8 handlers), ScheduleCallModal. TypeScript zero errors. (PR: #5049)
- **2026-05-08** — Trader Cockpit end-to-end: complete automated meat trader workflow with Dependency Approval Engine (validates Customer/Supplier/Plant/Contact/Carrier/Warehouse), TradePipelineViewSet API (/trades/initiate/, /trades/active/, /trades/{id}/advance/), TraderCockpitPage with New Trade wizard + active trades grid, DependencyWizard with quick-create, TradePipelineTracker visual step indicator. Route at /trader-cockpit. (PR: #5047)
- **2026-05-08** — Cold Storage & Carriers pages: replaced ComingSoon placeholders with production pages — Carriers with insurance compliance + EntityFormSurface CRUD; Cold Storage with temperature zones, capacity cards, FIFO lot inventory, expiry alerts. (PR: #5045)
- **2026-05-08** — Universal Form Enhancements: expanded _SKIP_FIELDS to hide audit/soft-delete fields, enhanced _infer_field_group for intelligent section inference (Order Details, Shipping & Delivery, Billing & Payment, Contact Information, Dates & Scheduling), _normalize_schema now injects ui.section from computed group, added field deduplication, dedicated Sales Order schema with sections/allow_create. Frontend DynamicFormEngine renders section headers between field groups, adds "+ Add New" option for FK fields with allow_create, applies ui.default_value defaults, and skips hidden fields. (PRs: #5042, #5043)
- **2026-05-08** — Fix email sync on login: Django user_logged_in signal triggers sync_single_tenant Celery task (countdown=3) for tenants with active Microsoft provider. Frontend emits sync status events, AIAgentWidget shows "Syncing emails…" pill indicator. (PR: #5041)
- **2026-05-08** — AI Widget WebSocket resilience: dev-only structured logging for connect/close/error/retry, wrapped refreshAccessToken in try/catch, added close code/reason logging. Zero production console output. (PR: #5040)
- **2026-05-08** — Fix stale Phase 16b documentation: updated acceptance criteria from "planned only" → "fully shipped". (PR: #5039)
