...
- **2026-04-29** — CI/docs: deepen drift-gate coverage so CURRENT docs fail on nonexistent workflow references, wrong branch-protection names, obsolete PR checks, and forbidden Golden Pipeline guidance; align workflow README, branch-protection setup, and reference Golden Pipeline docs to the live workflow topology. (PR: #4765)
- **2026-04-29** — CI: replace reusable deploy’s inline required-secret lists with manifest-driven lane validation via `config/manage_env.py` + `.github/scripts/validate-environment.sh`, enforce the shared contract in `validate-workflows.sh`, and remove duplicated required/optional secret arrays from `manifests/env.manifest.json` so backend/frontend deploy requiredness stays aligned with the canonical manifest. (PR: #4764)
...
- **2026-04-29** — Backend contracts: add explicit OpenAPI request/response coverage for high-churn AI assistant endpoints (`/ai-chat/chat/`, `/chat/`, `/metrics/`, `/review/pending/`, `/tools/openapi/`, `/errors/recent/`, `/diagnostics/recent-errors/`, `/review/{feedback_id}/resolve/`, `/swarm/invoke/`), add wrapper serializers for previously ad hoc payloads, stabilize enum naming via `SPECTACULAR_SETTINGS`, and update the OpenAPI baseline to the shipped AI contract surface. (PR: #4775)
- **2026-04-29** — Frontend/mobile contracts: add root `openapi-typescript` generation for `manifests/openapi/openapi-schema.baseline.json`, commit shared generated + curated TS contract aliases under `shared/types/`, adopt generated AI assistant types in the frontend service/widget flow, and adopt generated tenant workform list/detail contracts in mobile with runtime guards around `workflow_definition`. (PR: #4777)
- **2026-04-29** — Frontend tenant safety: add shared tenant-aware React Query key helpers backed by the canonical tenant ID utility, migrate the first EH-04.1 customer/supplier/permissions query surfaces onto tenant-scoped cache keys, keep `/health/` global, and replace blanket query cache clears with targeted eviction of legacy non-tenant caches on tenant switch. (PR: #4779)
- **2026-04-29** — Frontend search contracts: add a shared normalized search service/type layer, route CommandPalette, SmartSearch, and ContinuousSearch through the same frontend search contract, normalize entity type aliases consistently (`po`/`so` to canonical types), and replace the dead frontend-only continuous-search assumption with universal search plus type filtering. (PR: #4780)
- **2026-04-29** — Frontend FlowEditor decomposition: extract the inline `flowEditorNodeActions` adapter from `UnifiedFlowEditor.tsx` into a dedicated `useFlowEditorNodeActions` hook, preserve node override/container insertion semantics, and add focused hook regression coverage so future FlowEditor slicing can proceed behind a stable seam. (PR: #4782)
- **2026-04-29** — Runtime/ops readiness: require Redis-backed cache and channel readiness for non-dev backend lanes, make staging/production fail closed without `REDIS_URL`, gate reusable deploy + smoke on `/api/v1/ready/` for UAT/production, propagate the manifest-defined Redis secret through backend mutation/runtime paths, and update Golden validators/docs so the non-dev readiness contract cannot silently drift. (PR: #4783)
- **2026-04-29** — Backend workflow collaboration: replace naive node lock acquisition with guarded atomic cache semantics, add tenant-scoped workflow lock/renew REST actions matching the existing frontend hook paths, and add focused service/websocket/fail-closed regressions so lock contention is deterministic without cross-tenant bleed. (PR: #4785)
- **2026-04-29** — Ops governance: require explicit immutable rollback refs for UAT/production, document a reproducible UAT rollback drill plus lane-wide observability ownership in the canonical runbooks, and extend Golden-state enforcement so rollback/observability guidance cannot drift silently. (PR: #4787)
- **2026-04-29** — AI autonomy foundation: add tenant-aware AIRun/AITask/AIApproval persistence with additive RLS-backed migration, expose tenant-scoped run/task/approval endpoints plus owner/admin approve/deny actions, gate the first governed mutating flow (`draft_vendor_email`) behind approval, and refresh the OpenAPI baseline plus regression coverage for queryability, idempotency, and unauthenticated fail-closed behavior. (PR: #4789)
- **2026-04-29** — AI semantic lineage: add tenant-safe semantic chunk + lineage persistence with additive RLS migration, index parsed documents and inject semantic context into the chat runtime, harden Microsoft Graph attachment retries, move RLHF compiled exports to durable Django storage, surface semantic readiness in health checks, and refresh the OpenAPI baseline plus focused regression coverage. (PR: #4792)
...
- **2026-04-29** — CI: add PR-time dependency review plus workflow/Dockerfile security linting, restrict Dependabot auto-merge to explicit dependency manifests/lockfiles only, and enforce the narrowed scope + critical CODEOWNERS overrides in the workflow validator. (PR: #4766)
- **2026-04-29** — Docs/autonomy: ship the missing golden planning artifacts (`.github/TECH_DEBT_REGISTER.md`, `.github/SDLC_PROTOCOLS.md`, `.github/EPIC_TICKETS.md`, `.cursorrules`), append the continuation protocol to `.github/copilot-instructions.md`, and sync EH-01 ticket state so EH-01.4 is the next real ready item for autonomous continuation. (PR: #4767)
- **2026-04-29** — Ops/docs: align `.github/scripts/deployment-rollback.sh` with the live reusable deploy filesystem + port contract, add direct health verification to rollback checks, and document the current rollback/release governance path in the incident runbook, Golden Pipeline doc, and workflow README while release creation remains manual. (PR: #4769)
- **2026-04-29** — Docs/backlog: sync `.github/EPIC_TICKETS.md` after EH-01.4 so the canonical first ready ticket moves to EH-02.1 (`fail-closed-tenant-rls-runtime`) instead of pointing at already-merged work. (PR: #4770)
- **2026-04-29** — Backend tenant safety: make `TenantMiddleware` fail closed with a stable 503 when tenant RLS session variables cannot be asserted, abort the integrations OAuth callback before tenant-scoped writes if `set_current_tenant()` fails, and remove remaining fail-open tenant RLS contexts from invitation/core task paths with regression coverage. (PR: #4771)
- **2026-04-29** — Docs/backlog: sync `.github/EPIC_TICKETS.md` after EH-02.1 so the canonical next ready ticket moves to EH-02.2 (`platform-idempotency-keys`) instead of the already-merged fail-closed RLS batch. (PR: #4772)
...
- **2026-04-29** — Backend: add tenant-scoped idempotency protection for `POST /api/v1/tenant-workforms/{id}/execute/` by introducing `core.IdempotencyKey` with additive RLS coverage, a reusable core reserve/replay service, cached response replay for duplicate keys, stable 409 rejection for mismatched payload reuse, and regression coverage that proves duplicate execute requests do not create multiple execution rows or enqueue duplicate tasks. (PR: #4773)
- **2026-04-29** — Backend: migrate AI chat persistence from JSON-stamped tenant context to tenant-native storage by adding additive tenant FKs + backfill + RLS policies for `ChatSession`/`ChatMessage`, upgrading chat/document/email-ingestion writers to stamp tenant directly, and extending the audit/RLS registry coverage for the new tenant-scoped chat tables. (PR: #4774)
- **2026-04-29** — Ops/Backend: enforce non-dev Redis/Valkey readiness by propagating cache/channel envs through reusable deploy, making `/api/v1/ready/` fail when non-dev falls back to LocMem/InMemory, switching backend deploy/smoke gates to `/api/v1/ready/`, and documenting the temporary lane override valve via `REDIS_REQUIRED`. (PR: #4784)
...
- **2026-04-30** — Docs/backlog reconciliation: narrowed the WorkForms runtime “next” note to the still-open form `fields` model cleanup and queued `EH-04.5 workforms-fields-model-cleanup` as the new first ready ticket after verifying clean `development` had no unchecked backlog items. (PR: #4805)
- **2026-04-30** — Frontend WorkForms: canonicalized form-node saves to `formFields`, migrated active preview/inference/mapping/runtime readers onto the shared dual-model resolver, preserved backward-compatible legacy `fields` loads, and added focused regression coverage for save normalization, load hydration, config writes, and output schema inference. (PR: #4806)
- **2026-04-30** — Backend deploy hotfix: resolved the `tenant_apps.ai_assistant` Django test discovery collision by removing the `tests.py` / `tests/` naming conflict, restoring backend test execution for the blocked development deploy pipeline so the previously failed PR batches can ship in the next successful `development` deployment. (PR: #4807)
- **2026-04-30** — Backend contract hotfix: restored OpenAPI back-compat for the deploy hotfix PR by pinning `ProteinTypeChoices` to the shipped `TypeOfProteinEnum` component name and adding a drf-spectacular post-processing alias so legacy `ProteinEnum` references remain present during schema generation. (PR: #4807)
