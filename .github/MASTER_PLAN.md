...
- **2026-05-12** — UX-37.3 Cockpit workspace search/catalog cleanup: replaced the Cockpit SmartSearch forwarding stub with static Command Center search guidance, removed `AILearningMetricsWidget` from new widget-catalog additions, and kept saved layouts render-safe for existing AI Learning Metrics widgets. Validation used `npm -C frontend run verify-standards`, focused `CockpitDashboard.test.tsx`, and the full frontend `test:ci` suite. (PR: pending)
