...
- **2026-04-28** — WorkForms runtime: add tenant-scoped execution telemetry (`ExecutionEventLog`) with RLS, persist normalized audit-trail events for executions/node spans/action outcomes, and add backend regression coverage for successful and failed action events. (PR: TBD)
