...
- **2026-04-28** — Enterprise reliability & real-time parity: rolled soft-delete coverage out to primary tenant master-data models, added JWT-secured tenant mutation broadcasts plus a React Query invalidation hook for live entity views, and shipped additive semantic AI indexing (`AIDocumentChunk`, vector-backed tenant memory retrieval) with pgvector-aligned dev/CI Postgres provisioning. (PR: #33)
- **2026-04-28** — Phase 3 & 8: added versioned tenant/global caching for system choices, tenant config, products, and universal-search reads; aligned Cmd+K to the canonical universal search service-layer path with direct `/records/:entityType/:id` navigation; and exposed a Graph tab backed by the core entity-graph endpoint on the record page. (PR: #34)
- **2026-04-28** — Plants hotfix: moved UniversalEntityForm and DynamicFormEngine async schema/config/option loading onto React Query, stabilized cascading product-option queries by parent signature, and added focused regressions so Edit Plant no longer re-enters the fetch/render loop that triggered React error #185. (PR: #35)
