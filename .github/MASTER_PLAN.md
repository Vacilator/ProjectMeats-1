...
- **2026-05-06** — Phase 15 settlement execution: shipped `B2B-03.3 reconciliation-engine-into-paymenttransaction`, adding additive settlement-event review/linkage fields plus migration coverage in `tenant_apps.integrations`; a new deterministic reconciliation service that exact-matches invoice, sales-order, and purchase-order references into the canonical `PaymentTransaction` ledger while preserving replay safety, explicit review reason codes, and tenant-scoped row locking; retry-safe task behavior so `READY_TO_POST` review events keep their reason codes; and focused backend coverage for exact matches, ambiguous matches, amount mismatches, replay retries, and cross-tenant failures. This promotes `B2B-03.4 accounting-settlement-queue-and-override-ui` as the next ready Phase 15 ticket. Validation: `bash scripts/verify_golden_state.sh`; `cd backend && source /venv/bin/activate && python manage.py makemigrations --check`; `cd backend && source /venv/bin/activate && python manage.py migrate --plan`; `cd backend && source /venv/bin/activate && python manage.py test tenant_apps.integrations.tests.SettlementTaskTests --keepdb -v 2`; `cd backend && source /venv/bin/activate && python manage.py test tenant_apps.integrations tenant_apps.invoices tenant_apps.sales_orders tenant_apps.purchase_orders --keepdb -v 2`. (PR: #4911)
