# How-To Guides

> Step-by-step guides for common tasks.
>
> **Last Updated**: 2026-02-01

## Development

| Guide | Purpose |
|-------|---------|
| [Development Workflow](DEVELOPMENT_WORKFLOW.md) | Daily development process |
| [Design System](DESIGN_SYSTEM.md) | UI/UX standards and components |
| [User Training](USER_TRAINING_GUIDE.md) | End-user training materials |

## Configuration

| Guide | Purpose |
|-------|---------|
| [Allowed Hosts](ALLOWED_HOSTS_CONFIGURATION.md) | Django allowed hosts setup |
| [Branch Protection](BRANCH_PROTECTION_SETUP.md) | GitHub branch rules |

## Database

| Guide | Purpose |
|-------|---------|
| [Database Sync Guide](DATABASE_SYNC_GUIDE.md) | Production → UAT sync (consolidated) |

## Email

| Guide | Purpose |
|-------|---------|
| [Email Configuration](EMAIL_CONFIGURATION.md) | SendGrid setup & troubleshooting (consolidated) |

## SSL & Security

| Guide | Purpose |
|-------|---------|
| [SSL Configuration](SSL_CONFIGURATION.md) | Let's Encrypt & zero-downtime renewal (consolidated) |

## Operations

| Guide | Purpose |
|-------|---------|
| [Deployment Emergency Fix](DEPLOYMENT_EMERGENCY_FIX.md) | Emergency procedures |
| [Django Staff Permissions](DJANGO_STAFF_PERMISSIONS_EXPLAINED.md) | Admin permissions |
